...
class TournamentUsageRepository:
    ...
    async def get_recent_tournaments(
        ...
        """
        usages = (
            await TournamentUsage.filter(user=user)
            .order_by("-last_accessed")
            .limit(limit)
        )

        logger.debug(
            "Retrieved %d recent tournaments for user %s", len(usages), user.id
        )
    ...
    async def clear_tournament_usage(self, tournament_id: int) -> int:
        ...
        """
        deleted_count = await TournamentUsage.filter(
            tournament_id=tournament_id
        ).delete()

        logger.info(
            "Cleared %d usage records for tournament %s", deleted_count, tournament_id
        )
    ...
    async def clear_user_usage(self, user_id: int) -> int:
        ...
        logger.info("Cleared %d usage records for user %s", deleted_count, user_id)
    ...
    async def cleanup_excess_per_user(self, keep_per_user: int = 10) -> int:
        ...
        users_with_usage = (
            await TournamentUsage.all().distinct().values_list("user_id", flat=True)
        )
        ...
        for user_id in users_with_usage:
            ...
            all_usage = (
                await TournamentUsage.filter(user_id=user_id)
                .order_by("-last_accessed")
