...
class OrganizationRepository:
    ...
    async def create(
        self, name: str, description: Optional[str] = None, is_active: bool = True
    ) -> Organization:
    ...
    async def is_user_org_admin(self, user_id: int, organization_id: int) -> bool:
        ...
        """
        member = await (
            OrganizationMember.filter(user_id=user_id, organization_id=organization_id)
    ...
    async def list_members(self, organization_id: int) -> list[OrganizationMember]:
        ...
        return await (
            OrganizationMember.filter(organization_id=organization_id)
    ...
    async def get_member(
        ...
        return await (
            OrganizationMember.filter(organization_id=organization_id, user_id=user_id)
            .prefetch_related("user", "permissions")
            .first()
        )
    ...
    async def list_memberships_for_user(self, user_id: int) -> list[OrganizationMember]:
        ...
        return await (
            OrganizationMember.filter(user_id=user_id)
    ...
    async def list_permissions(
        ...
        return await OrganizationPermission.filter(
    ...
    async def get_or_create_permission(
        ...
        perm = await OrganizationPermission.get_or_none(
            organization_id=organization_id, permission_name=permission_name
        )
        if perm:
            ...
            if description is not None and perm.description != description:
                ...
                await perm.save()
            return perm
        return await OrganizationPermission.create(
            organization_id=organization_id,
    ...
    async def get_permissions_by_names(
        ...
        if not name_list:
            return []
        return await OrganizationPermission.filter(
            organization_id=organization_id, permission_name__in=name_list
        )

    async def delete_permission(
        self, organization_id: int, permission_name: str
    ) -> int:
    ...
    async def set_permissions_for_member(
        ...
        to_add = [
            p for name, p in target_by_name.items() if name not in current_by_name
        ]
        to_remove = [
            p for name, p in current_by_name.items() if name not in target_by_name
        ]
        if to_remove:
    ...
    async def add_member(
        ...
        """
        existing = await OrganizationMember.get_or_none(
            organization_id=organization_id, user_id=user_id
        )
        if existing:
            return existing
