...
class RacetimeApiService:
    ...
    def __init__(self):
        ...
        self.token_refresh_buffer = timedelta(minutes=5)  # Refresh if expiring within 5 minutes
    ...
    async def _ensure_valid_token(self, user: User) -> str:
        ...
        if user.racetime_token_expires_at:
            ...
            if user.racetime_token_expires_at <= now + self.token_refresh_buffer:
                logger.info("RaceTime token for user %s is expired or expiring soon, refreshing", user.id)
                
                # Check if we have a refresh token
                if not user.racetime_refresh_token:
                    logger.warning("User %s has expired token but no refresh token", user.id)
                    raise ValueError("RaceTime token is expired and no refresh token available")
                
                # Refresh the token
                user_service = UserService()
                user = await user_service.refresh_racetime_token(user)
                logger.info("Successfully refreshed RaceTime token for user %s", user.id)
    ...
    async def get_past_races(
        ...
        if user.racetime_id:
            ...
            for race in races:
                ...
                if any(e.get('user', {}).get('id') == user.racetime_id for e in entrants):
