...
async def sync_discord_scheduled_events() -> dict:
    ...
    for tournament in tournaments:
        try:
            ...
            # Sync events for this tournament
            ...
            logger.debug(
                "Tournament %s sync complete: %d created, %d updated, %d deleted",
                tournament.id,
                stats.get('created', 0),
                stats.get('updated', 0),
                stats.get('deleted', 0),
    ...
    logger.info(
        "Discord scheduled events sync complete: %d created, %d updated, %d deleted, %d errors",
        total_created,
        total_updated,
        total_deleted,
        total_errors,
