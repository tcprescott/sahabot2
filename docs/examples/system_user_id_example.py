    ...
    if is_authenticated_user_id(event.user_id):
        ...
        if event.match_id:
            ...
            try:
                ...
                logger.error(
                    "Failed to record result for user %s: %s", event.user_id, e
                )
        else:
...
async def send_discord_notification(event: RacetimeEntrantStatusChangedEvent):
    ...
    logger.info(
        "Sending Discord notification to user %s for race finish", event.user_id
    )
...
async def handle_racetime_bot_command(
    racetime_user_id: str, command: str, race_room: str
):
    """
    ...
    if command.startswith("!forceforfeit"):
        ...
        if user and user.has_permission(Permission.ADMIN):
            ...
            logger.warning(
                "User %s attempted privileged command without permission", user_id
            )
