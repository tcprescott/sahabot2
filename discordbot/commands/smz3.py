...
class SMZ3Commands(commands.Cog):
    ...
    async def smz3_generate(
        ...
        try:
            ...
            if preset:
                try:
                    ...
                    if preset_obj and preset_obj.settings:
                        ...
                        preset_info = (
                            f"Warning: Preset '{preset}' not found, using defaults\n"
                        )
                except Exception as e:
                    logger.error("Error loading preset %s: %s", preset, e)
                    preset_info = (
                        f"Warning: Could not load preset '{preset}', using defaults\n"
                    )
            ...
            if preset_info:
                embed.add_field(
                    name="Settings", value=preset_info.strip(), inline=False
                )

            # Add spoiler URL if generated
            if spoiler and result.spoiler_url:
                embed.add_field(
                    name="Spoiler Log", value=result.spoiler_url, inline=False
                )
            ...
            if result.metadata:
                ...
                if details:
                    embed.add_field(
                        name="Configuration", value=" | ".join(details), inline=False
                    )
            ...
            await interaction.followup.send(
                f"❌ Error generating SMZ3 seed: {str(e)}", ephemeral=True
            )

    @app_commands.command(
        name="smz3_presets", description="List available SMZ3 presets"
    )
    async def smz3_presets(self, interaction: discord.Interaction):
        ...
        try:
            ...
            presets = await self.preset_service.list_presets(
                randomizer="smz3", is_public=True
            )
            ...
            await interaction.followup.send(
                f"❌ Error listing presets: {str(e)}", ephemeral=True
            )
