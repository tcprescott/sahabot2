...
class SMCommands(commands.Cog):
    ...
    @app_commands.command(
        name="smvaria", description="Generate a Super Metroid VARIA seed"
    )
    ...
    async def smvaria(
        ...
        try:
            ...
            await interaction.followup.send(
                f"❌ Failed to generate VARIA seed: {str(e)}", ephemeral=True
            )

    @app_commands.command(
        name="smdash", description="Generate a Super Metroid DASH seed"
    )
    @app_commands.describe(
        preset="Preset name (optional, default: standard)",
    ...
    async def smdash(
        ...
        try:
            ...
            await interaction.followup.send(
                f"❌ Failed to generate DASH seed: {str(e)}", ephemeral=True
            )

    @app_commands.command(
        name="smtotal", description="Generate a Super Metroid total randomization seed"
    )
    @app_commands.describe(
    ...
    async def smtotal(
        ...
        try:
            ...
            logger.info(
                "Generating total randomization seed for user %s", interaction.user.id
            )
            ...
            embed.add_field(name="Preset", value=preset, inline=True)
            embed.add_field(
                name="Features", value="Area + Boss + Major/Minor Split", inline=False
            )
