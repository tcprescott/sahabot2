"""Service layer for Async Qualifier management.
...
class LeaderboardEntry:
    ...
    user: User
    races: List[Optional[AsyncQualifierRace]]
...
class AsyncQualifierService:
    """Business logic for async qualifiers with organization scoping."""

    def __init__(self) -> None:
        self.repo = AsyncQualifierRepository()
    ...
    async def can_manage_async_tournaments(
        ...
        """Check if user can manage async qualifiers in the organization."""
        if not user:
            return False
    ...
    async def can_review_async_races(
        ...
        """
        Check if user can review async qualifier races.
    ...
    async def list_org_tournaments(
        ...
        """List async qualifiers for an organization after access check."""
    ...
    async def list_active_org_tournaments(
        ...
        if not is_member:
            logger.warning(
                "Non-member user %s attempted to list active async qualifiers for org %s",
                getattr(user, "id", None),
                organization_id,
            )
            return []
    ...
    async def get_tournament(
        self, user: Optional[User], organization_id: int, qualifier_id: int
        ...
        return await self.repo.get_by_id(qualifier_id, organization_id)
    ...
    ) -> Tuple[Optional[AsyncQualifier], List[str]]:
        """
    ...
    async def update_tournament(
        self, user: Optional[User], organization_id: int, qualifier_id: int, **fields
    ) -> Tuple[Optional[AsyncQualifier], List[str]]:
        """
        Update a tournament.
        ...
        current_tournament = await self.repo.get_by_id(qualifier_id, organization_id)
    ...
    async def delete_tournament(
        ...
        return await self.repo.delete(qualifier_id, organization_id)
    ...
    async def create_pool(
        ...
        pool = await self.repo.create_pool(
            qualifier_id, organization_id, name, description
        )
        if pool:
            await self.repo.create_audit_log(
                qualifier_id=qualifier_id,
    ...
    async def get_user_races(
        ...
        return await self.repo.list_races(
            qualifier_id, organization_id, user_id=user.id
        )
    ...
    async def get_permalink(
        ...
        if permalink.pool.tournament.id != qualifier_id:
            return None
    ...
    async def get_permalink_races(
        ...
        permalink = await self.get_permalink(
            user, organization_id, qualifier_id, permalink_id
        )
        if not permalink:
            return []
    ...
    async def get_active_races_for_user(
        self, user: User, organization_id: int, qualifier_id: int
    ) -> List[AsyncQualifierRace]:
    ...
    async def create_race(
        ...
        has_in_progress = await self.repo.has_in_progress_race(user.id, qualifier_id)
        if has_in_progress:
            logger.warning(
    ...
    async def calculate_permalink_scores(
        ...
        races = await AsyncQualifierRace.filter(
            permalink_id=permalink_id,
            status="finished",
            reattempted=False,
        )
    ...
    async def calculate_tournament_scores(
        ...
        async with score_calculation_lock:
            tournament = await self.repo.get_by_id(qualifier_id, organization_id)
            if not tournament:
                return False
            ...
            logger.info("Recalculated all scores for tournament %s", qualifier_id)
            return True
    ...
    async def get_leaderboard(
        ...
        for user_id in user_ids:
            races_list: List[Optional[AsyncQualifierRace]] = []
    ...
    async def get_reattempt_count(
        ...
        """
        # Verify access
        tournament = await self.get_tournament(user, organization_id, qualifier_id)
        if not tournament:
            return 0

        # Count races marked as reattempted by this user
        count = await self.repo.count_reattempted_races(user.id, qualifier_id)
        logger.debug(
    ...
    async def can_reattempt_race(
        ...
        qualifier_check = await self.get_tournament(
            user, organization_id, tournament.id
        )
        if not qualifier_check:
