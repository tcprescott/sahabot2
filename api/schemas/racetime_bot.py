...
class RacetimeBotOut(BaseModel):
    ...
    status: int = Field(
        ...,
        description="Bot connection status (0=UNKNOWN, 1=CONNECTED, 2=AUTH_FAILED, 3=CONNECTION_ERROR, 4=DISCONNECTED)",
    )
    ...
    last_status_check_at: Optional[datetime] = Field(
        None, description="Last status check timestamp"
    )
...
class RacetimeBotCreate(BaseModel):
    ...
    category: str = Field(
        ..., description="RaceTime category slug", min_length=1, max_length=50
    )
    name: str = Field(
        ..., description="Friendly name for the bot", min_length=1, max_length=255
    )
...
class RacetimeBotUpdate(BaseModel):
    ...
    category: Optional[str] = Field(
        None, description="RaceTime category slug", min_length=1, max_length=50
    )
    name: Optional[str] = Field(
        None, description="Friendly name for the bot", min_length=1, max_length=255
    )
