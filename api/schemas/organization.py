...
class OrganizationListResponse(BaseModel):
    ...
    items: list[OrganizationOut] = Field(
...
class OrganizationCreateRequest(BaseModel):
    ...
    is_active: bool = Field(
        True, description="Whether the organization should be active"
    )
...
class OrganizationPermissionListResponse(BaseModel):
    ...
    items: list[OrganizationPermissionOut] = Field(
...
class OrganizationPermissionCreateRequest(BaseModel):
    ...
    permission_name: str = Field(
        ..., min_length=1, max_length=100, description="Permission name"
    )
    description: Optional[str] = Field(None, description="Permission description")
...
class OrganizationMemberOut(BaseModel):
    ...
    permissions: list[str] = Field(
        ...
        description="List of permission names assigned to this member",
    )
...
class OrganizationMemberListResponse(BaseModel):
    ...
    items: list[OrganizationMemberOut] = Field(
...
class OrganizationMemberPermissionsUpdateRequest(BaseModel):
    ...
    permission_names: list[str] = Field(
        ..., description="List of permission names to assign to the member"
    )
