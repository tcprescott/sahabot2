...
class OrganizationInviteOut(BaseModel):
    ...
    expires_at: Optional[datetime] = Field(None, description="Invite expiration timestamp (null = never expires)")
...
class OrganizationInviteListResponse(BaseModel):
    ...
    items: list[OrganizationInviteOut] = Field(..., description="List of invite objects")
...
class OrganizationInviteUseResponse(BaseModel):
    ...
    organization_id: Optional[int] = Field(None, description="Organization ID joined (if successful)")
