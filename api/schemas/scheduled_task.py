...
class ScheduledTaskBase(BaseModel):
    ...
    interval_seconds: Optional[int] = Field(
        None, description="For INTERVAL type: interval in seconds"
    )
    cron_expression: Optional[str] = Field(
        None, description="For CRON type: cron expression"
    )
    scheduled_time: Optional[datetime] = Field(
        None, description="For ONE_TIME type: specific time to run"
    )
