...
class OrganizationSettingsView:
    ...
    async def _open_edit_dialog(self) -> None:
        ...
        dialog = OrganizationDialog(
            organization=self.organization, on_save=self._refresh
        )
        await dialog.show()
        ...
        with Card.create(title="Organization Information"):
            with ui.column().classes("gap-md"):
                with ui.row().classes("w-full justify-between items-center"):
                    ...
                    ui.button(
                        "Edit", icon="edit", on_click=self._open_edit_dialog
                    ).classes("btn")
    ...
    async def _render_org_settings(self) -> None:
        ...
        with Card.create(title="Organization Settings", classes="mt-2"):
            with ui.row().classes("w-full justify-between"):
                ...
                ui.button("Add Setting", icon="add", on_click=open_create).props(
                    "color=positive"
                ).classes("btn")
            ...
            def render_actions(s):
                with ui.element("div").classes("flex gap-2"):
                    ...
                    async def open_edit() -> None:
                        await self._open_edit_setting(
                            s.key, s.value, s.description or ""
                        )
                    ...
                    ui.button("Delete", icon="delete", on_click=delete_item).classes(
                        "btn"
                    )
    ...
    async def _open_create_setting(self) -> None:
        dialog = OrgSettingDialog(
            organization_id=self.organization.id, on_save=self._refresh
        )
        await dialog.show()
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    with ui.row().classes("dialog-actions"):
                        ...
                        async def do_delete() -> None:
                            await self.settings_service.delete_org(
                                self.organization.id, key
                            )
                            confirm.close()
                            await self._refresh()
