"""
Async tournament admin view for managing racetime chat commands at the ASYNC_TOURNAMENT level.

Tournament managers can configure custom chat commands specific to their async tournament.
"""

from nicegui import ui
from components.data_table import ResponsiveTable, TableColumn
from components.dialogs import RacetimeChatCommandDialog, ConfirmDialog
from models import User, RacetimeChatCommand, Organization, AsyncTournament, CommandResponseType
...
class AsyncTournamentRacetimeChatCommandsView:
    """Async tournament-scoped racetime chat commands view."""

    def __init__(self, current_user: User, org: Organization, tournament: AsyncTournament):
        """
        Initialize the async tournament racetime chat commands view.

        Args:
            current_user: Currently authenticated user
            org: Organization context
            tournament: Async tournament context
        """
    ...
    async def render(self):
        """Render the async tournament racetime chat commands interface."""
        with ui.column().classes('full-width gap-md'):
            # Header section
            with ui.element('div').classes('card'):
                with ui.element('div').classes('card-header'):
                    ui.label('Async Tournament Chat Commands').classes('text-xl font-bold')
                with ui.element('div').classes('card-body'):
                    ui.label(
                        f'Configure custom chat commands for {self.tournament.name}. '
                        'These commands are only available in racetime rooms for this async tournament.'
    ...
    async def _load_commands(self):
        """Load and display commands for the async tournament."""
        # Fetch commands
        self.commands = await self.command_service.get_commands_for_async_tournament(
            self.current_user,
            self.tournament.id,
        )
            ...
            with self.table_container:
                with ui.element('div').classes('card'):
                    with ui.element('div').classes('card-body text-center'):
                        ...
                        ui.label('No custom commands for this async tournament').classes('text-secondary')
    ...
    async def _add_command(self):
        ...
        dialog = RacetimeChatCommandDialog(
            async_tournament_id=self.tournament.id,
