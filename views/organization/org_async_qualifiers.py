"""
Organization async qualifiers view.

List, create, edit, and delete async qualifiers within an organization.
"""
...
class OrganizationAsyncQualifiersView:
    """Manage async qualifiers for an organization."""
    ...
    async def _refresh(self) -> None:
        """Re-render the async qualifiers list."""
        if self.container:
    ...
    async def _open_create_dialog(self) -> None:
        """Open dialog to create a new async qualifier."""
        ...
        async def on_save(data: dict) -> None:
            ...
            if tournament:
                ...
                ui.notify("Failed to create async qualifier", type="negative")

        dialog = AsyncQualifierDialog(
            tournament=None, on_save=on_save, discord_channels=discord_channels
        )
        await dialog.show()

    async def _open_edit_dialog(self, tournament: AsyncQualifier) -> None:
        """Open dialog to edit an existing async qualifier."""
        # Get available Discord channels
        discord_channels = await self._get_discord_channels()
        ...
        async def on_save(data: dict) -> None:
            ...
            if updated:
                ...
                ui.notify("Failed to update async qualifier", type="negative")
    ...
    async def _confirm_delete(self, tournament: AsyncQualifier) -> None:
        """Ask for confirmation and delete the async qualifier if confirmed."""
        ...
        async def do_delete() -> None:
            ...
            if success:
                ...
                ui.notify("Failed to delete async qualifier", type="negative")

        dialog = ConfirmDialog(
            title="Delete async qualifier",
    ...
    async def _check_channel_permissions(
        self, tournament: AsyncQualifier, force_recheck: bool = False
    ) -> list[str]:
        """
    ...
    async def _post_embed_to_discord(self, tournament: AsyncQualifier) -> None:
        ...
        try:
            ...
            import discord
            from discordbot.async_tournament_views import AsyncQualifierMainView

            # Create the embed
            embed = discord.Embed(
                title=tournament.name,
                description=tournament.description or "Async Qualifier",
            ...
            await channel.send(embed=embed, view=AsyncQualifierMainView())

            ui.notify(
                "Tournament embed posted to Discord successfully!", type="positive"
            )
            logger.info(
                "Posted async qualifier embed for tournament %s to channel %s",
                tournament.id,
                tournament.discord_channel_id,
            )
    ...
    async def _render_content(self) -> None:
        """Render async qualifiers list and actions."""
        ...
        async def recheck_permissions(tournament: AsyncQualifier) -> None:
        ...
        with Card.create(title="Async Qualifiers"):
            with ui.row().classes("w-full justify-between mb-2"):
                ui.label(f"{len(tournaments)} async qualifier(s) in this organization")
                ui.button(
                    "New async qualifier",
                    icon="add",
            ...
            if not tournaments:
                with ui.element("div").classes("text-center mt-4"):
                    ...
                    ui.label("No async qualifiers yet").classes("text-secondary")
                    ui.label('Click "New async qualifier" to create one').classes(
                        "text-secondary text-sm"
                    )
            else:

                def render_active(t: AsyncQualifier):
                    if t.is_active:
                ...
                def render_discord_channel(t: AsyncQualifier):
                ...
                def render_permissions_warnings(t: AsyncQualifier):
                ...
                def render_runs_per_pool(t: AsyncQualifier):
                    ui.label(str(t.runs_per_pool))

                def render_actions(t: AsyncQualifier):
    ...
    async def render(self) -> None:
        """Render the async qualifiers view."""
