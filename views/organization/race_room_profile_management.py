...
from components.dialogs.tournaments.race_room_profile_dialog import (
    RaceRoomProfileDialog,
)
...
class RaceRoomProfileManagementView:
    ...
    async def render(self):
        ...
        with ui.element("div").classes("card"):
            ...
            with ui.element("div").classes("card-body"):
                ui.label(
                    "Manage reusable RaceTime room configuration profiles"
    ...
    async def _render_bots_card(self):
        ...
        bots = await bot_service.get_bots_for_organization(
        ...
        with ui.element("div").classes("card"):
            ...
            with ui.element("div").classes("card-body"):
                if not bots:
                    ...
                    ui.label(
                        f'{len(bots)} bot{"s" if len(bots) != 1 else ""} assigned to this organization'
                    ).classes("text-sm text-grey mb-4")
        ...
        with self.table_container:
            if not self.profiles:
                with ui.element("div").classes("text-center p-8 text-grey"):
                    ...
                    ui.label(
                        "Create your first race room profile to get started"
                    ).classes("text-sm")
                return
            ...
            columns = [
                TableColumn(
                    label="Name", cell_render=lambda p: self._render_name_cell(p)
                ),
                TableColumn(
                    label="Settings",
                ...
                TableColumn(
                    label="Default", cell_render=lambda p: self._render_default_cell(p)
                ),
                TableColumn(
                    label="Actions", cell_render=lambda p: self._render_actions_cell(p)
                ),
            ]
    ...
    def _render_settings_cell(self, profile):
        ...
        with ui.column().classes("gap-1"):
            ui.label(
                f"Start: {profile.start_delay}s, Limit: {profile.time_limit}h"
            ).classes("text-sm")
            settings = []
    ...
    def _render_actions_cell(self, profile):
        ...
        with ui.row().classes("gap-2"):
            if not profile.is_default:
                ui.button(
                    icon="star_outline", on_click=lambda p=profile: self._set_default(p)
                ).props("flat dense round").tooltip("Set as Default")

            ui.button(
                icon="edit", on_click=lambda p=profile: self._edit_profile(p)
            ).props("flat dense round").tooltip("Edit")

            ui.button(
                icon="delete", on_click=lambda p=profile: self._delete_profile(p)
    ...
    async def _delete_profile(self, profile):
        ...
        async def confirm_delete():
            ...
            success = await service.delete_profile(
                self.user, self.organization.id, profile.id
            )

            if success:
                ui.notify(
                    f"Profile '{profile.name}' deleted successfully!", type="positive"
                )
                await self._render_table()
            else:
