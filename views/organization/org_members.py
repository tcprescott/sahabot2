...
class OrganizationMembersView:
    ...
    async def _open_permissions_dialog(self, member) -> None:
        ...
        current_perms = await self.service.list_member_permissions(
    ...
    async def _render_invite_links(self) -> None:
        ...
        invites = await self.invite_service.list_org_invites(
        ...
        with Card.create(title="Invite Links"):
            ...
            if not invites:
                with ui.element("div").classes("text-center mt-4"):
                    ...
                    ui.label("Create a link to share with new members").classes(
                ...
                def render_invite_url(inv):
                    ...
                    with ui.row().classes("items-center gap-2"):
                        ...
                        ui.button(
                            icon="content_copy", on_click=copy_to_clipboard
                        ).props("flat dense")
                    ...
                    with ui.element("div").classes("flex gap-2"):
                        if self.can_manage:
                            if inv.is_active:
                                ...
                                ui.button(
                                    "Deactivate", icon="link_off", on_click=deactivate
                                ).classes("btn")
                            else:
                                ...
                                ui.button(
                                    "Activate", icon="link", on_click=activate
                                ).classes("btn")

                            async def delete_invite():
                                async def do_delete():
                                    await self.invite_service.delete_invite(
                                        self.user, self.organization.id, inv.id
                                    )
                                    await self._refresh()
                            ...
                            ui.button(
                                "Delete", icon="delete", on_click=delete_invite
    ...
    async def _open_create_invite_dialog(self) -> None:
        ...
        async def on_submit(slug, max_uses, expires_at):
            _invite, error = await self.invite_service.create_invite(
                self.user, self.organization.id, slug, max_uses, expires_at
            )
            if error:
        ...
        with Card.create(title="Organization Members", classes="mt-2"):
            with ui.row().classes("w-full justify-between mb-2"):
                ...
                if self.can_manage:
                    ui.button(
                        "Invite Member",
                        icon="person_add",
                        on_click=self._open_invite_dialog,
                    ).props("color=positive").classes("btn")
            ...
            if not members:
                with ui.element("div").classes("text-center mt-4"):
                    ...
                    ui.label("Add members to get started").classes(
                        "text-secondary text-sm"
                    )
            else:
                ...
                async def render_permissions(m):
                    perms = (
                        await m.permissions.all() if hasattr(m, "permissions") else []
                    )
                    if perms:
                    ...
                    with ui.element("div").classes("flex gap-2"):
                        if self.can_manage:
                            ui.button(
                                "Edit Permissions",
                                icon="security",
                                on_click=lambda m=m: self._open_permissions_dialog(m),
                            ).classes("btn")
                        else:
    ...
    async def render(self) -> None:
        ...
        self.can_manage = await self.ui_auth.can_manage_members(
            self.user, self.organization.id
        )
