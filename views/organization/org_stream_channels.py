...
class OrganizationStreamChannelsView:
    ...
    async def _open_create_dialog(self) -> None:
        ...
        async def on_submit(name: str, stream_url: Optional[str], is_active: bool) -> None:
            await self.service.create_channel(self.user, self.organization.id, name, stream_url, is_active)
            await self._refresh()
    ...
    async def _open_edit_dialog(self, channel) -> None:
        ...
        async def on_submit(name: str, stream_url: Optional[str], is_active: bool) -> None:
            await self.service.update_channel(self.user, self.organization.id, channel.id, name=name, stream_url=stream_url, is_active=is_active)
            await self._refresh()
    ...
    async def _confirm_delete(self, channel) -> None:
        ...
        async def do_delete() -> None:
            await self.service.delete_channel(self.user, self.organization.id, channel.id)
            await self._refresh()
    ...
    async def _render_content(self) -> None:
        ...
        with Card.create(title='Stream Channels'):
            with ui.row().classes('w-full justify-between mb-2'):
                ...
                ui.button('New Channel', icon='add', on_click=self._open_create_dialog).props('color=positive').classes('btn')

            if not channels:
                with ui.element('div').classes('text-center mt-4'):
                    ui.icon('cast').classes('text-secondary icon-large')
                    ui.label('No stream channels yet').classes('text-secondary')
                    ui.label('Click "New Channel" to create one').classes('text-secondary text-sm')
            else:
                def render_url(c):
                ...
                def render_actions(c):
                    with ui.element('div').classes('flex gap-2'):
                        ...
                        ui.button('Delete', icon='delete', on_click=lambda c=c: self._confirm_delete(c)).classes('btn').props('color=negative')
