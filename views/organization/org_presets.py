        ...
        with ui.card().classes('p-4 hover:shadow-lg transition-shadow'):
            ...
            with ui.element('div').classes('flex justify-between items-start mb-2'):
                with ui.element('div').classes('flex-1'):
                    ...
                    ui.label(self.RANDOMIZER_LABELS.get(preset.randomizer, preset.randomizer)).classes('text-sm badge badge-secondary')

                # Public badge
                if preset.is_public:
                    ui.icon('public', size='sm').classes('text-secondary').tooltip('Public preset')

            # Description
            if preset.description:
                with ui.element('div').classes('mb-2'):
                    ui.label(preset.description).classes('text-sm text-secondary line-clamp-2')
            ...
            with ui.element('div').classes('flex gap-2'):
                ...
                ui.button('View', icon='visibility', on_click=view_preset).classes('btn flex-1').props('dense')
                ...
                if can_edit:
                    ...
                    ui.button('Edit', icon='edit', on_click=edit_preset).classes('btn flex-1').props('dense color=primary')

                    # Delete button
                    async def delete_preset(p=preset):
                        await self._delete_preset(p)

                    ui.button('Delete', icon='delete', on_click=delete_preset).classes('btn').props('dense color=negative icon-only')
