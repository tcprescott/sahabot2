...
from components.dialogs.organization.racer_verification_dialog import (
...
class RacerVerificationConfigView:
    ...
    async def render(self):
        ...
        self.verifications = await self.service.get_verifications_for_organization(
            current_user=self.user, organization_id=self.organization_id
        )
        ...
        with ui.element("div").classes("card"):
            ...
            with ui.element("div").classes("card-header"):
                with ui.row().classes("w-full items-center justify-between"):
                    ...
                    ui.button(icon="add", on_click=self._open_create_dialog).classes(
                        "btn btn-primary"
                    ).tooltip("Create Verification")

            # Body
            with ui.element("div").classes("card-body"):
                if not self.verifications:
                    ui.label("No racer verifications configured.").classes(
                        "text-secondary"
                    )
                    ui.label(
                    ...
                    self.table = ResponsiveTable(
                        columns=columns, rows=self.verifications
                    )
    ...
    def _render_actions(self, row):
        ...
        with ui.row().classes("gap-1"):
            ui.button(
                icon="edit", on_click=lambda r=row: self._open_edit_dialog(r)
            ).classes("btn btn-sm").tooltip("Edit")
            ui.button(
                icon="delete", on_click=lambda r=row: self._confirm_delete(r)
    ...
    async def _delete_verification(self, verification):
        ...
        result = await self.service.delete_verification(
            current_user=self.user, verification_id=verification.id
        )
