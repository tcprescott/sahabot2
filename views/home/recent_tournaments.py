...
class RecentTournamentsView:
    ...
    async def render(self):
        ...
        recent_tournaments = await self.usage_service.get_recent_tournaments(self.user, limit=5)
        ...
        with Card.create(title='Recent Tournaments'):
            if not recent_tournaments:
                with ui.element('div').classes('text-center py-4'):
                    ...
                    ui.label('Visit tournaments to see them here').classes('text-secondary text-sm')
            else:
                                ...
                                with ui.row().classes('w-full items-center justify-between'):
                                    with ui.column().classes('gap-1 flex-grow'):
                                        ...
                                        with ui.row().classes('gap-2 items-center'):
                                            ui.icon('business').classes('text-sm text-secondary')
                                            ui.label(tournament_info['organization_name']).classes('text-sm text-secondary')
                                        
                                        # Last accessed time
                                        with ui.row().classes('gap-2 items-center'):
                                            ui.icon('schedule').classes('text-sm text-secondary')
                                            ui.label('Last visited:').classes('text-xs text-secondary')
                                            DateTimeLabel.datetime(tournament_info['last_accessed'], classes='text-xs')
