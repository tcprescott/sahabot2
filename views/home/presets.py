    ...
    async def _render_content(self) -> None:
        ...
        with Card.create(title="Randomizer Presets"):
            ...
            with ui.element("div").classes("flex justify-between items-start mb-4"):
                with ui.element("div"):
                    ui.label(
                        "Browse randomizer presets from your namespace, global presets, and other public namespaces."
                    )
                    ui.label(
                        "Use filters below to show only your namespace or explore all public presets."
                    ).classes("text-sm text-secondary")

                # Create preset button
                async def create_preset():
                    """Open dialog to create a new preset."""
                    from components.dialogs.organization.preset_editor_dialog import (
                        PresetEditorDialog,
                    )

                    dialog = PresetEditorDialog(user=self.user, on_save=self._refresh)
                    await dialog.show()

                ui.button("Create Preset", icon="add", on_click=create_preset).classes(
                    "btn"
                    ...
                    with ui.element("div").classes("card"):
                        ...
                        async def render_randomizer_cell(preset):
                            randomizer_label = self.RANDOMIZER_LABELS.get(
                                preset.randomizer, preset.randomizer
                            )
                        ...
                        async def render_scope_cell(preset):
                            if preset.namespace:
                                with ui.row().classes("items-center gap-1"):
                                    ...
                                    ui.label(preset.namespace.display_name).classes(
                                        "text-sm"
                                    )
                            else:
                                with ui.row().classes("items-center gap-1"):
                        ...
                        async def render_creator_cell(preset):
                            if preset.user:
                                ui.label(preset.user.get_display_name()).classes(
                                    "text-sm"
                                )
                            else:
                        ...
                        async def render_visibility_cell(preset):
                            visibility_class = (
                                "badge-success" if preset.is_public else "badge-warning"
                            )
                            visibility_text = (
                                "Public" if preset.is_public else "Private"
                            )
                            with ui.element("span").classes(
                                f"badge {visibility_class}"
                            ):
                        ...
                        async def render_actions_cell(preset):
                            with ui.row().classes("gap-1"):
                                ...
                                ui.button(
                                    icon="visibility", on_click=view_yaml
                                ).classes("btn btn-sm").props("flat").tooltip(
                                    "View YAML"
                                )

                                # Edit and delete buttons (only if user owns preset or is SUPERADMIN)
                                can_edit = (
                                    preset.user_id == self.user.id
                                    or self.user.has_permission(Permission.SUPERADMIN)
                                )
                                ...
                                if can_edit:
                                    ...
                                    ui.button(
                                        icon="edit", on_click=edit_preset
                                    ...
                                    async def delete_preset():
                                        ...
                                        dialog = ConfirmDialog(
                                            ...
                                            on_confirm=lambda: self._delete_preset(
                                                preset.id
                                            ),
                                        )
                                        await dialog.show()
                                    ...
                                    ui.button(
                                        icon="delete", on_click=delete_preset
                                    ).classes("btn btn-sm").props(
                                        "flat color=negative"
                                    ).tooltip(
                                        "Delete Preset"
                        ...
                        columns = [
                            ...
                            TableColumn(
                                label="Randomizer", cell_render=render_randomizer_cell
                            ),
                            TableColumn(label="Scope", cell_render=render_scope_cell),
                            TableColumn(
                                label="Creator", cell_render=render_creator_cell
                            ),
                            TableColumn(
                                label="Visibility", cell_render=render_visibility_cell
                            ),
                            TableColumn(
                                label="Created", cell_render=render_created_cell
                            ),
                            TableColumn(
                                label="Actions", cell_render=render_actions_cell
                            ),
                        ]
                ...
                with ui.element("div").classes("card"):
                    with ui.element("div").classes("card-body"):
                        with ui.row().classes("items-center gap-2"):
                            ...
                            with ui.column():
                                ui.label(f"Failed to load presets: {str(e)}").classes(
