"""
Async Qualifier Dashboard View.

Shows a player's own races in an async qualifier with stats and progress.
"""
...
class AsyncDashboardView:
    """View for a player's async qualifier dashboard."""
        ...
        with Card.create(title=self.tournament.name):
            ...
            if self.tournament.discord_channel_id:
                with ui.element("div").classes("mt-2"):
                    ...
                    ui.link(
                        "Go to Tournament Discord Channel", discord_url, new_tab=True
                    ).classes("btn-link")

    async def _render_player_stats(
        self, races: list[AsyncQualifierRace], pools
    ):  # noqa: ARG002
        """Render player statistics card."""
        # Calculate stats
        completed_races = [
            r for r in races if r.status == "finished" and not r.reattempted
        ]
        ...
        # Best race
        best_race = (
            min(completed_races, key=lambda r: r.elapsed_time.total_seconds())
            if completed_races
            else None
        )
        ...
        with Card.create(title="My Statistics"):
            ...
            if best_race:
                with ui.element("div").classes("mt-4"):
                    ...
                    ui.label(f"{best_race.elapsed_time_formatted}").classes(
                        "inline-block text-success"
                    )
                    await best_race.fetch_related("permalink", "permalink__pool")
                    ui.label(f" on {best_race.permalink.pool.name}").classes(
            ...
            if reattempted:
                with ui.element("div").classes("mt-2"):
                    ...
                    ui.label("You have used your reattempt").classes(
                ...
                for stats in pool_stats.values():
                    ...
                    with ui.element("div").classes("pool-progress-card"):
                        ...
                        with ui.element("div").classes("progress-bar-container"):
                            with ui.element("div").classes("progress-bar").style(
                                f"width: {percent}%"
                            ):
                                pass
                        ...
                        if stats["remaining"] == 0:
                            ...
                            ui.label(f'{stats["remaining"]} remaining').classes(
                                "text-sm mt-1"
                            )
    ...
    async def _render_races_table(self, races: list[AsyncQualifierRace]):
        ...
        with Card.create(title="My Races"):
            ...
            with ui.element("div").classes("flex flex-wrap gap-md mb-4"):
                ...
                status_filter = ui.select(
                    status_options, value="All", label="Status Filter"
                ...
                sort_select = ui.select(
                    sort_options, value="Newest First", label="Sort By"
                ).classes("filter-select")
            ...
            async def refresh_races():
                ...
                with races_container:
                    await self._render_races_content(
                        races, status_filter.value, sort_select.value
                    )
    ...
    async def _render_races_content(
        self, races: list[AsyncQualifierRace], status_filter: str, sort_by: str
    ):
        ...
        if sort_by == "Newest First":
            filtered_races = sorted(
                filtered_races, key=lambda r: r.created_at, reverse=True
            )
        ...
        if not filtered_races:
            with ui.element("div").classes("text-center py-4"):
                ui.label("No races match the selected filters").classes(
                    "text-secondary"
                )
            return
    ...
    def _render_date(self, race: AsyncQualifierRace):
        """Render date cell."""
    ...
    def _render_status(self, race: AsyncQualifierRace):
        """Render status cell."""
    ...
    def _render_score(self, race: AsyncQualifierRace):
        """Render score cell."""
        score_text = f"{race.score:.1f}" if race.score is not None else "—"
        ui.label(score_text)

    def _render_vod(self, race: AsyncQualifierRace):
        """Render VOD cell."""
    ...
    def _render_actions(self, race: AsyncQualifierRace):
        """Render actions cell."""
        with ui.row().classes("gap-sm"):
            # View permalink
            ui.link("View Seed", race.permalink.url, new_tab=True).classes(
                "btn-link"
            )  # External link
            ...
            if race.discord_thread_id:
                thread_url = (
                    f"https://discord.com/channels/@me/{race.discord_thread_id}"
                )
                ui.link("Thread", thread_url, new_tab=True).classes("btn-link")

    async def _render_race_card_mobile(self, race: AsyncQualifierRace):
            ...
            with ui.element("div").classes("card-body"):
                ...
                with ui.element("div").classes("grid grid-cols-2 gap-md mb-3"):
                    ...
                    with ui.element("div"):
                        ...
                        score_text = (
                            f"{race.score:.1f}" if race.score is not None else "—"
                        )
                ...
                with ui.element("div").classes("flex flex-wrap gap-sm"):
                    ...
                    if race.runner_vod_url:
                        ui.link("Watch VOD", race.runner_vod_url, new_tab=True).classes(
                            "btn btn-sm"
                        )
                    ...
                    if race.discord_thread_id:
                        ...
                        ui.link("Discord Thread", thread_url, new_tab=True).classes(
                            "btn btn-sm"
                        )
