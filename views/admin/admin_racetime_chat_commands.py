"""
Admin view for managing racetime chat commands at the BOT level.

Global administrators can configure chat commands that are available
across all racetime rooms managed by a specific bot instance.
"""
...
class AdminRacetimeChatCommandsView:
    """Admin view for racetime chat commands."""

    def __init__(self, current_user: User):
        """
        Initialize the admin racetime chat commands view.
    ...
    async def render(self):
        """Render the racetime chat commands administration interface."""
        with ui.column().classes('full-width gap-md'):
            # Header section
            with ui.element('div').classes('card'):
                with ui.element('div').classes('card-header'):
                    ui.label('RaceTime Chat Commands').classes('text-xl font-bold')
                with ui.element('div').classes('card-body'):
                    ui.label(
                        'Configure chat commands that respond to ! prefixed messages in racetime.gg rooms. '
                        'Commands can provide static text or dynamic responses via handlers.'
                    ).classes('text-secondary')

            # Bot selection and filters
            with ui.element('div').classes('card'):
                with ui.element('div').classes('card-body'):
                    with ui.row().classes('w-full gap-md items-end'):
                        # Bot selector
                        self.bots = await self.bot_service.get_all_bots(self.current_user)
                        bot_options = {
                            bot.bot_name: bot.id for bot in self.bots
                        }
                        ui.select(
                            label='RaceTime Bot',
                            options=bot_options,
                            value=self.bots[0].id if self.bots else None,
                            on_change=lambda e: self._on_bot_change(e.value)
            ...
            # Load initial data
            if self.bots:
                self.selected_bot_id = self.bots[0].id
                await self._load_commands()

    async def _load_commands(self):
        """Load and display commands for the selected bot."""
        if not self.selected_bot_id:
            return

        # Fetch commands
        self.commands = await self.command_service.get_commands_for_bot(
            self.current_user,
    ...
    async def _on_bot_change(self, bot_id: int):
    ...
    async def _add_command(self):
        ...
        if not self.selected_bot_id:
            ui.notify('Please select a bot first', color='negative')
            return

        dialog = RacetimeChatCommandDialog(
            bot_id=self.selected_bot_id,
