...
class ScheduledTasksView:
    ...
    def _render_task_type(self, task_info: dict) -> None:
        ...
        type_name = (
            TaskType(task_type).name if isinstance(task_type, int) else task_type.name
        )
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    with ui.column().classes("gap-2 mb-4"):
                        with ui.row().classes("items-center gap-2"):
                            ...
                            ui.label(f"Task ID: {task_info['task_id']}").classes(
                                "text-sm font-mono"
                            )
                    ...
                    if task_info["last_error"]:
                        ...
                        with ui.row().classes(
                            "items-center gap-2 p-3 rounded bg-negative text-white"
                        ):
                            ui.icon("error")
                    ...
                    with ui.row().classes("justify-end gap-2 mt-4"):
                        ui.button(
                            "Refresh",
                            on_click=lambda: (dialog.close(), self._refresh()),
                        ).classes("btn")
    ...
    async def _execute_builtin_task_now(self, task_info: dict) -> None:
        ...
        success = await TaskSchedulerService.execute_builtin_task_now(
            self.user, task_id
        )

        if success:
            ui.notify(
                f'Built-in task "{task_name}" execution triggered', type="positive"
            )
            # Refresh after a short delay to allow task to update status
            await self._refresh()
        else:
            ui.notify(
                "Failed to execute task - check permissions or task status",
                type="negative",
            )
    ...
    async def _execute_db_task_now(self, task) -> None:
        ...
        success = await self.service.execute_task_now(
            self.user, task.organization_id, task.id  # Will be None for global tasks
        )
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    with ui.column().classes("gap-2 mb-4"):
                        ...
                        with ui.row().classes("items-center gap-2"):
                            ...
                            ui.label("Source: Database (Global)").classes(
                                "text-sm badge"
                            )
                    ...
                    with ui.column().classes("gap-2"):
                        with ui.row().classes("items-center gap-2"):
                            ...
                            if task.last_run_at:
                                DateTimeLabel.create(
                                    task.last_run_at, format_type="relative"
                                )
                            else:
                                ui.label("Never").classes("text-secondary text-sm")
                        ...
                        with ui.row().classes("items-center gap-2"):
                            ...
                            if task.next_run_at:
                                DateTimeLabel.create(
                                    task.next_run_at, format_type="relative"
                                )
                            else:
                                ui.label("—").classes("text-secondary text-sm")
                        ...
                        with ui.column().classes("gap-2"):
                            if task.created_at:
                                with ui.row().classes("items-center gap-2"):
                                    ...
                                    DateTimeLabel.create(
                                        task.created_at, format_type="datetime"
                                    )
                            if task.created_by_id:
                                with ui.row().classes("items-center gap-2"):
                                    ui.label("Created By:").classes("text-sm font-bold")
                                    ui.label(f"User ID {task.created_by_id}").classes(
                                        "text-sm"
                                    )
    ...
    async def _toggle_builtin_task(self, task_info: dict) -> None:
        ...
        if success:
            ...
            ui.notify(
                "Failed to toggle task status - check permissions", type="negative"
            )
    ...
    def _render_actions(self, task_info: dict) -> None:
        ...
        with ui.element("div").classes("flex gap-2"):
            ...
            ui.button(
                ...
                f'color={"warning" if is_active else "positive"} size=sm'
            )
            ...
            ui.button(
                "Details", icon="info", on_click=make_details_handler(task_info)
    ...
    async def _render_content(self) -> None:
            ...
            with ui.element("div").classes("card-header"):
                with ui.row().classes("w-full justify-between items-center"):
                    with ui.column():
                        ...
                        ui.label(
                            f'Task Scheduler: {"Running" if is_running else "Stopped"}'
                        ).classes(
                            "text-sm text-positive"
                            if is_running
                            else "text-sm text-negative"
                        )
                    ui.button(
                        "Refresh", icon="refresh", on_click=self._refresh
                    ).classes("btn")

        # Built-in Tasks Card
        with Card.create(title="Built-in Tasks"):
            ui.label("System tasks defined in code and loaded at startup.").classes(
            ...
            success_count = sum(
                1 for t in builtin_tasks if t["last_status"] == "success"
            )
        ...
        with Card.create(title="Global Database Tasks"):
            ui.label("User-created global tasks stored in the database.").classes(
                "text-sm text-secondary mb-4"
            )
            ...
            if db_tasks:
                ...
                db_success_count = sum(
                    1 for t in db_tasks if t.last_run_status == "success"
                )
                db_failed_count = sum(
                    1 for t in db_tasks if t.last_run_status == "failed"
                )
                ...
                with ui.row().classes("items-center gap-2 p-4 bg-secondary/10 rounded"):
                    ...
                    ui.label("No global database tasks created yet.").classes(
                        "text-secondary"
                    )
