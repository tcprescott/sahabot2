...
class PresetNamespacesView:
    ...
    async def _render_content(self) -> None:
        ...
        with Card.create(title="Preset Namespaces"):
            ...
            with ui.element("div").classes("mb-4"):
                ...
                ui.label("Namespaces can be owned by users or organizations.").classes(
            ...
            try:
                ...
                with ui.element("div").classes(
                    "p-4 rounded bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"
                ):
                    ui.icon("error").classes("mr-2")
                    ui.label(f"Failed to load namespaces: {str(e)}").classes(
                        "font-bold"
                    )
    ...
    async def _render_namespaces_table(self, namespaces: list[PresetNamespace]) -> None:
        ...
        columns = [
            TableColumn(
                label="Name", cell_render=lambda ns: self._render_name_cell(ns)
            ),
            TableColumn(label="Display Name", key="display_name"),
            TableColumn(
                label="Owner", cell_render=lambda ns: self._render_owner_cell(ns)
            ),
            TableColumn(
                label="Visibility",
            ...
            TableColumn(
                label="Actions", cell_render=lambda ns: self._render_actions_cell(ns)
            ),
        ]
    ...
    def _render_owner_cell(self, namespace: PresetNamespace) -> None:
        ...
        if namespace.user:
            ui.badge(f"User: {namespace.user.get_display_name()}").props(
                "color=primary"
            )
    ...
    def _render_actions_cell(self, namespace: PresetNamespace) -> None:
        ...
        with ui.row().classes("gap-2"):
            ...
            ui.button("View", icon="visibility", on_click=view_namespace).classes(
                "btn"
            ).props("flat dense")
            ...
            ui.button("Delete", icon="delete", on_click=delete_namespace).classes(
                "btn"
    ...
    async def _delete_namespace(self, namespace: PresetNamespace) -> None:
        ...
        message = (
            f'Are you sure you want to delete namespace "{namespace.display_name}"?'
        )
        if preset_count > 0:
        ...
        async def confirm_delete():
            ...
            try:
                ...
                ui.notify(
                    f'Namespace "{namespace.display_name}" deleted', type="positive"
                )
                await self._refresh()
            except Exception as e:
                logger.error(
                    "Failed to delete namespace %s: %s", namespace.id, e, exc_info=True
                )
                ui.notify(f"Failed to delete namespace: {str(e)}", type="negative")

        # Show confirmation dialog
        dialog = ConfirmDialog(
            title="Delete Namespace", message=message, on_confirm=confirm_delete
        )
        await dialog.show()
