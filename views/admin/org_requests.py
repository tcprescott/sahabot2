        ...
        with ui.column().classes("w-full gap-4") as self.container:
            ...
            with ui.row().classes("items-center justify-between w-full"):
                with ui.column().classes("gap-1"):
                    ...
                    ui.label(
                        "Review and approve requests for new organizations."
            ...
            with ui.element("div").classes("card-body"):
                if not pending_requests:
                    ...
                    async def render_requested_by_cell(request):
                        ui.label(request.requested_by.discord_username).classes(
                            "text-sm"
                        )
                    ...
                    async def render_actions_cell(request):
                        with ui.row().classes("gap-1"):
                            ...
                            ui.button(icon="check_circle", on_click=approve).classes(
                                "btn btn-sm"
                            ).props("flat color=positive").tooltip("Approve Request")

                            # Reject button
                            async def reject():
                                await self._show_reject_dialog(request)

                            ui.button(icon="cancel", on_click=reject).classes(
                                "btn btn-sm"
                            ).props("flat color=negative").tooltip("Reject Request")

                    columns = [
                        TableColumn(
                            label="Organization Name", cell_render=render_name_cell
                        ),
                        TableColumn(
                            label="Requested By", cell_render=render_requested_by_cell
                        ),
                        TableColumn(
                            label="Requested At", cell_render=render_requested_at_cell
                        ),
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    async def render_reviewed_by_cell(request):
                        if request.reviewed_by:
                            ui.label(request.reviewed_by.discord_username).classes(
                                "text-sm"
                            )
                        else:
                    ...
                    reviewed_columns = [
                        TableColumn(
                            ...
                            cell_render=render_reviewed_name_cell,
                        ),
                        TableColumn(label="Status", cell_render=render_status_cell),
                        TableColumn(
                            label="Reviewed By", cell_render=render_reviewed_by_cell
                        ),
                        TableColumn(
                            label="Reviewed At", cell_render=render_reviewed_at_cell
                        ),
                    ]

                    reviewed_table = ResponsiveTable(
                        columns=reviewed_columns, rows=reviewed_requests
                    )
