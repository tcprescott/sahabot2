...
class PresetsView:
    ...
    def __init__(self, user: User, service: RandomizerPresetService):
        """
    ...
    async def _render_content(self) -> None:
        ...
        with Card.create(title="Global Randomizer Presets"):
            ...
            with ui.element("div").classes("mb-4"):
                ui.label(
                    "Create and manage global YAML presets for randomizer seed generation."
                )
                ui.label(
                    "Global presets are available to all users and don't belong to any namespace."
            ...
            with ui.element("div").classes("flex flex-wrap gap-2 mb-4 items-center"):
                ...
                async def open_create_dialog():
                    dialog = PresetEditorDialog(
                        user=self.user, is_global=True, on_save=self._refresh
                    )
                    await dialog.show()

                ui.button(
                    "Create Global Preset", icon="add", on_click=open_create_dialog
        ...
        try:
            ...
            if not presets:
                ...
                with ui.element("div").classes("card"):
                    with ui.element("div").classes("card-body text-center py-8"):
                        ...
                        ui.label("No global presets found").classes(
                            "text-lg font-bold mt-2"
                        )
                        ui.label("Create a global preset to get started").classes(
            ...
            with ui.element("div").classes("card"):
                ...
                async def render_actions_cell(preset):
                    with ui.row().classes("gap-1"):
                        ...
                        ui.button(icon="visibility", on_click=view_preset).classes(
                            "btn btn-sm"
                        ).props("flat").tooltip("View YAML")
                        ...
                        if can_edit:
                            ...
                            async def edit_preset():
                                dialog = PresetEditorDialog(
                                    user=self.user, preset=preset, on_save=self._refresh
                                )
                                await dialog.show()

                            ui.button(icon="edit", on_click=edit_preset).classes(
                                "btn btn-sm"
                            ).props("flat").tooltip("Edit")

                            # Delete button
                            async def delete_preset():
                                await self._delete_preset(preset)

                            ui.button(icon="delete", on_click=delete_preset).classes(
                                "btn btn-sm"
                            ).props("flat color=negative").tooltip("Delete")
            ...
            with ui.element("div").classes("card"):
                with ui.element("div").classes("card-body"):
                    ui.label(f"Error loading presets: {str(e)}").classes(
                        "text-negative"
                    )
    ...
    async def _delete_preset(self, preset) -> None:
        ...
        async def on_confirm():
            try:
                ...
                logger.error(
                    "Error deleting preset %s: %s", preset.id, e, exc_info=True
                )
