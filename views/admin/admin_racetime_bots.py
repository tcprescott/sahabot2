...
class AdminRacetimeBotsView:
        ...
        with ui.column().classes("full-width gap-md"):
            ...
            with ui.element("div").classes("card"):
                ...
                with ui.element("div").classes("card-body"):
                    ui.label(
                        "Configure RaceTime.gg bot instances and assign them to organizations."
                ...
                with ui.element("div").classes("card-body"):
                    with ui.row().classes(
                        ...
                        with ui.row().classes("items-center gap-3"):
                            ...
                            # Refresh button
                            ui.button(
                                icon="refresh", on_click=self._refresh_bots
                            ).classes("btn").props("flat")

                            # Add bot button
                            ui.button(
                                "Add Bot", icon="add", on_click=self._open_add_bot
    ...
    async def _render_table(self):
        ...
        for bot in self.bots:
            orgs = await self.bot_service.get_organizations_for_bot(
                bot.id, self.current_user
            )
            rows.append(
                {
                    "bot": bot,
        ...
        def render_client_id(row):
            bot = row["bot"]
            client_id_display = (
                bot.client_id[:20] + "..." if len(bot.client_id) > 20 else bot.client_id
            )
        ...
        def render_actions(row):
            ...
            with ui.row().classes("gap-1"):
                ui.button(
                    icon="edit", on_click=lambda b=bot: self._open_edit_bot(b.id)
                ).props("flat round dense size=sm").tooltip("Edit Bot")

                # Restart button (only for active bots)
                if bot.is_active:
                    ui.button(
                        icon="refresh", on_click=lambda b=bot: self._restart_bot(b.id)
                ...
                ui.button(
                    ...
                    f"Manage Organizations ({org_count})"
                )
                ui.button(
                    icon="delete", on_click=lambda b=bot: self._confirm_delete_bot(b.id)
    ...
    async def _open_add_bot(self):
        ...
        dialog = RacetimeBotAddDialog(
            current_user=self.current_user, on_save=self._refresh_bots
        )
        await dialog.show()
    ...
    async def _open_edit_bot(self, bot_id: int):
        ...
        dialog = RacetimeBotEditDialog(
            bot=bot, current_user=self.current_user, on_save=self._refresh_bots
        )
        await dialog.show()
    ...
    async def _confirm_delete_bot(self, bot_id: int):
        ...
        dialog = ConfirmDialog(
            title="Confirm Delete", message=message, on_confirm=do_delete
        )
        await dialog.show()
