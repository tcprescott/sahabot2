...
class AdminLogsView:
        ...
        with ui.column().classes("full-width gap-md"):
            ...
            with ui.element("div").classes("card"):
                with ui.element("div").classes("card-body"):
                    with ui.row().classes("full-width gap-4 items-center flex-wrap"):
                        ...
                        auto_scroll_switch.on(
                            "update:model-value", self._on_auto_scroll_toggle
                        )
            ...
            with ui.element("div").classes("card"):
                with ui.element("div").classes("card-body"):
                    ...
                    ui.label(
                        f"Showing {count} log records (max {self.log_handler.max_records})"
                    ).classes("text-sm text-secondary mb-2")

                    # Log container with scrolling
                    with ui.element("div").classes(
                        "border rounded p-2 bg-gray-50 log-scroll-container"
                    ) as scroll_area:
    ...
    async def _refresh_logs(self):
        ...
        records = self.log_handler.get_records(level=level, search=self.search_filter)
    ...
    def _render_log_record(self, record: LogRecord):
        ...
        with ui.row().classes("full-width gap-2 items-start"):
            ...
            # Logger name
            ui.label(record.logger_name).classes(
                "text-gray-600 text-xs log-logger-name"
            )

            # Message
            ui.label(record.message).classes(
                "text-gray-800 text-xs flex-grow log-message"
            )

        # Show exception info if present
        if record.exc_info:
            with ui.element("div").classes(
                "ml-8 mt-1 p-2 bg-red-50 border-l-4 border-red-500 rounded"
            ):
                ui.label(record.exc_info).classes(
                    "text-red-800 text-xs whitespace-pre-wrap log-exception"
                )
    ...
    async def _clear_logs(self):
        ...
        with confirm:
            with ui.element("div").classes("card"):
                ...
                with ui.element("div").classes("card-body"):
                    ui.label("Are you sure you want to clear all stored logs?").classes(
                        "mb-4"
                    )
                    with ui.row().classes("dialog-actions"):
                        ...
                        ui.button("Clear", on_click=do_clear).props(
                            "color=warning"
                        ).classes("btn")
        confirm.open()
    ...
    async def _download_logs(self):
        ...
        try:
            ...
            if success:
                ...
                ui.notify(
                    "Download failed - JavaScript not loaded yet, please refresh",
                    type="warning",
                )
        except TimeoutError:
