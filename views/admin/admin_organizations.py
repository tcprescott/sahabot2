...
class AdminOrganizationsView:
    ...
    async def _render_content(self) -> None:
        ...
        with Card.create(title="Organizations"):
            with ui.row().classes("w-full justify-between"):
                ...
                ui.button(
                    "Create Organization", icon="add", on_click=self._open_create
            ...
            columns = [
                ...
                TableColumn(
                    "Active",
                    cell_render=lambda o: (
                        ui.icon("check_circle").classes("text-positive")
                        if o.is_active
                        else ui.icon("cancel").classes("text-negative")
                    ),
                ),
                TableColumn(
                    "Created",
                    cell_render=lambda o: DateTimeLabel.datetime(o.created_at),
                ),
    ...
    def _actions_cell(self, org):
        ...
        with ui.element("div").classes("flex gap-2"):
            ui.button(
                "Edit", icon="edit", on_click=lambda: self._open_edit(org)
            ).classes("btn")
            ui.button(
                "Manage",
                icon="settings",
                on_click=lambda: ui.navigate.to(f"/orgs/{org.id}/admin"),
            ).classes("btn")
