...
class NotificationPreferencesView:
    ...
    async def _render_add_subscription_form(self):
        ...
        with ui.column().classes("gap-4 p-4"):
            ...
            async def add_subscription():
                try:
                    ...
                    if event_type_select.value in self.ORG_SCOPED_EVENTS:
                        if not self.user_organizations:
                            ui.notify(
                                "You must be a member of an organization to subscribe to this event",
                                type="warning",
                            )
                            return
            ...
            ui.button(
                "Add Subscription", on_click=add_subscription, icon="add"
    ...
    async def _render_subscriptions_table(self):
        ...
        columns = [
            ...
            TableColumn(
                ...
                cell_render=lambda row: ui.label(
                    "Discord DM"
                    if row.notification_method == NotificationMethod.DISCORD_DM
                    else "Email"
                ),
            ),
            TableColumn(
