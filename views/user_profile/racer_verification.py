...
class RacerVerificationView:
    ...
    async def _render_single_verification(self):
        ...
        eligibility = await self.service.check_user_eligibility(
            user=self.user, verification_id=self.verification_id
        )
        ...
        await self._render_verification_card(
            verification, eligibility, user_verification
        )
    ...
    async def _render_all_verifications(self):
        ...
        from application.repositories.organization_repository import (
        ...
        if not memberships:
            ui.label("You are not a member of any organizations.").classes(
                "text-secondary"
            )
            return
        ...
        for membership in memberships:
            verifications = await self.service.get_verifications_for_organization(
                current_user=self.user, organization_id=membership.organization_id
            )
        ...
        for verification in all_verifications:
            # Check eligibility
            eligibility = await self.service.check_user_eligibility(
                user=self.user, verification_id=verification.id
            )
    ...
    async def _render_verification_card(
        self, verification, eligibility, user_verification
    ):
        ...
        with ui.element("div").classes("card"):
            with ui.element("div").classes("card-header"):
                with ui.row().classes("w-full items-center justify-between"):
                    with ui.column().classes("gap-1"):
                        # Display categories
                        categories_str = (
                            ", ".join([cat.upper() for cat in verification.categories])
                            if verification.categories
                            else "N/A"
                        )
                        ui.label(categories_str).classes("text-lg font-bold")
                        ui.label(f"Role: {verification.role_name}").classes(
            ...
            with ui.element("div").classes("card-body"):
                ...
                with ui.column().classes("gap-2"):
                    ...
                    ui.label(f'Rules: {", ".join(rules)}').classes(
                        "text-sm text-secondary"
                    )

                # Error message
                if eligibility.get("error"):
                    ui.label(f'Error: {eligibility["error"]}').classes(
                        "text-negative mt-2"
                    )
                ...
                if user_verification and user_verification.is_verified:
                    # Already verified
                    with ui.column().classes("gap-2"):
                        ui.label("You have been verified for this role!").classes(
                            "text-positive"
                        )

                        if user_verification.role_granted:
                            ui.label("The Discord role has been granted.").classes(
                                "text-sm text-secondary"
                            )
                        else:
                            ui.label(
    ...
    async def _verify(self, verification):
        ...
        try:
            ...
            if result:
                ...
                ui.notify(
                    "Verification failed - you may not be eligible.", type="negative"
                )
