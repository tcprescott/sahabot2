        ...
        with Card.create(title="API Keys"):
            with ui.row().classes("w-full justify-between mb-4"):
                ...
                ui.button(
                    "Generate New Key",
                    icon="add_circle",
                    on_click=self._generate_new_key,
            ...
            if not tokens:
                with ui.element("div").classes("text-center mt-4"):
                    ...
                    ui.label("Generate a key to access the API").classes(
                ...
                def render_actions(t):
                    with ui.row().classes("gap-2"):
                        ui.button(
                            "Revoke",
                            icon="block",
                            on_click=lambda token=t: self._revoke_token(token.id),
                        ).props("color=negative").classes("btn")
