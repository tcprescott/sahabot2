"""
Async Tournament Review Queue view.
...
class AsyncReviewQueueView:
    """View for reviewing async tournament races."""

    def __init__(self, tournament: AsyncTournament, user: User, organization_id: int):
        """
        Initialize the review queue view.

        Args:
            tournament: The async tournament
            user: Current user
    ...
    async def render(self):
        ...
        with Card.create(title=f'Review Queue - {self.tournament.name}'):
            ui.label('Review and approve async race submissions').classes('text-secondary mb-4')
            ...
            with ui.row().classes('gap-md mb-4 w-full flex-wrap'):
                with ui.element('div').classes('flex-grow'):
                    ...
                    status_select = ui.select(
                        options=['all', 'pending', 'in_progress', 'finished', 'forfeit', 'disqualified'],
                ...
                ui.button('Refresh', on_click=self._load_races, icon='refresh').classes('btn-primary mt-6')
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    ui.notify(f'Failed to load review queue: {response.status_code}', type='negative')
                    self.races = []
