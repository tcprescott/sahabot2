...
class MySettingsView:
    ...
    def __init__(
        self, user: User, organization: Organization, service: TournamentService
    ):
    ...
    async def _render_content(self) -> None:
        ...
        tournament_registrations = (
            await self.service.list_user_tournament_registrations(
                self.organization.id, self.user.id
            )
        )
        registered_tournament_ids = {
            reg.tournament_id for reg in tournament_registrations
        }
        ...
        with Card.create(title="My Tournament Settings"):
                                ...
                                with ui.element("div").classes("card-body"):
                                    with ui.row().classes(
                                        ...
                                        with ui.column().classes("gap-1"):
                                            ui.label(tournament.name).classes(
                                                "font-bold"
                                            )
            ...
            with ui.element("div").classes("mb-6"):
                ...
                if not tournament_registrations:
                    with ui.element("div").classes("text-center mt-4"):
                        ui.icon("emoji_events").classes("text-secondary icon-large")
                        ui.label("Not registered for any tournaments").classes(
                            "text-secondary"
                        )
                        ui.label("Contact a tournament organizer to register").classes(
                                    ...
                                    with ui.row().classes(
                                        ...
                                        with ui.column().classes("gap-1"):
                                            ...
                                            with ui.row().classes("items-center gap-1"):
                                                ui.label("Registered:").classes(
                                                    "text-sm"
                                                )
                                                DateTimeLabel.date(
                                                    reg.created_at, classes="text-sm"
                                                )
            ...
            with ui.element("div").classes("mt-6"):
                ...
                ui.label(
                    "Coming soon: Configure notifications for match reminders, check-ins, and results"
                ).classes("text-secondary text-sm")

            with ui.element("div").classes("mt-6"):
                ui.label("Display Preferences").classes("text-lg font-bold mb-2")
                ui.label(
                    "Coming soon: Customize how matches and schedules are displayed"
                ).classes("text-secondary text-sm")
