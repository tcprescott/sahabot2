...
class MyMatchesView:
    ...
    def _filter_matches(self, match_players):
        ...
        for mp in match_players:
            ...
            if (
                ...
                and mp.match.tournament_id not in self.selected_tournaments
            ):
                continue
    ...
    async def _render_content(self) -> None:
        ...
        all_match_players = await self.service.list_user_matches(
        ...
        async def open_submit_match_dialog():
            ...
            dialog = SubmitMatchDialog(
                user=self.user, organization=self.organization, on_save=self._refresh
            )
            await dialog.show()
                ...
                with ui.row().classes("full-width items-center justify-between gap-4"):
                    ...
                    with ui.row().classes("items-center gap-4"):
                        ...
                        ui.select(
                            ...
                            multiple=True,
                            on_change=lambda e: self._on_tournament_filter_change(
                                e.value
                            ),
                        ).classes("min-w-[200px]").props("use-chips")
                    ...
                    # Submit button
                    ui.button(
                        "Submit Match", icon="add", on_click=open_submit_match_dialog
        ...
        with Card.create(
            title=f"My Matches ({len(match_players)}/{len(all_match_players)})"
        ):
            if not match_players:
                with ui.element("div").classes("text-center mt-4"):
                    ...
                    ui.label(
                        "Ask a tournament organizer to add you to a match"
                    ...
                    if opponents:
                        with ui.column().classes("gap-1"):
                            for opponent in opponents:
                                ui.label(opponent.user.get_display_name()).classes(
                                    "text-sm"
                                )
                    else:
                        ui.label("—").classes("text-secondary")
