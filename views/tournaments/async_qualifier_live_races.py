"""
Async Qualifier Live Races View.
...
class AsyncLiveRacesView:
    """View for async qualifier live races with management capabilities."""
    ...
    def __init__(self, user: User, tournament: AsyncQualifier):
        ...
        self.tournament_service = AsyncQualifierService()
    ...
    async def _render_races_table(self, races: list[AsyncQualifierLiveRace]):
        """Render races in a responsive table."""
        # Define columns
        columns = [
            TableColumn(
                label="Status", key="status", cell_render=self._render_status_badge
            ),
            TableColumn(
                label="Scheduled",
                key="scheduled_at",
                cell_render=self._render_scheduled,
            ),
            ...
            TableColumn(
                label="RaceTime.gg",
                key="racetime_url",
                cell_render=self._render_racetime_link,
            ),
        ]
        ...
        if self.can_manage:
            columns.append(
                TableColumn(label="Actions", key="id", cell_render=self._render_actions)
            )
    ...
    def _render_status_badge(self, race: AsyncQualifierLiveRace):
        """Render status badge."""
        status_variants = {
            "scheduled": "info",
            "pending": "warning",
    ...
    def _render_scheduled(self, race: AsyncQualifierLiveRace):
        """Render scheduled datetime."""
        with ui.element("div"):
            DateTimeLabel.create(race.scheduled_at, format_type="datetime")

    async def _render_pool(self, race: AsyncQualifierLiveRace):
        """Render pool name."""
        await race.fetch_related("pool")
        with ui.element("div"):
            ui.label(race.pool.name if race.pool else "N/A")

    def _render_racetime_link(self, race: AsyncQualifierLiveRace):
    ...
    def _render_actions(self, race: AsyncQualifierLiveRace):
        ...
        with ui.row().classes("gap-1"):
            ...
            if race.status not in ("finished", "cancelled"):
                ui.button(
                    icon="cancel", on_click=lambda r=race: self._confirm_cancel_race(r)
            ...
            if race.status in ("scheduled", "cancelled"):
                ui.button(
                    icon="delete", on_click=lambda r=race: self._confirm_delete_race(r)
    ...
    async def _view_eligible_participants(self, race: AsyncQualifierLiveRace):
    ...
    async def _confirm_cancel_race(self, race: AsyncQualifierLiveRace):
    ...
    async def _confirm_delete_race(self, race: AsyncQualifierLiveRace):
