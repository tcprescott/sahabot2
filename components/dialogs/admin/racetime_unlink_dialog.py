...
class RacetimeUnlinkDialog(BaseDialog):
    ...
    async def show(self):
        """Display the dialog."""
        self.create_dialog(
            title="Unlink RaceTime Account", icon="link_off", max_width="500px"
        )
    ...
    def _render_body(self):
        ...
        with ui.element("div").classes("w-full"):
            ...
            with ui.element("div").classes("w-full bg-red-50 p-3 rounded"):
                ui.label(
                    "This will unlink the RaceTime account from this user."
                ).classes("text-sm")
                ui.label(
                    "The user will need to re-link their account if they want to use RaceTime features."
