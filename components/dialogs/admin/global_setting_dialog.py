...
class GlobalSettingDialog(BaseDialog):
    ...
    def __init__(self, key: Optional[str] = None, value: Optional[Any] = None, description: Optional[str] = None, is_public: bool = False, on_save: Optional[Callable[[], Awaitable[None]]] = None) -> None:
        super().__init__()
        self.key = key or ""
        self.value = value
    ...
    async def show(self) -> None:
        title = 'Create Global Setting' if not self.key else f'Edit Global Setting: {self.key}'
    ...
    def _render_body(self) -> None:
        with self.create_form_grid(columns=2):
            with ui.element('div').classes('span-2'):
                self.key_input = ui.input(label='Key', value=self.key, placeholder='e.g., maintenance_mode').classes('w-full')
            with ui.element('div').classes('span-2'):
