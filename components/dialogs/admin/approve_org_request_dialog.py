...
class ApproveOrgRequestDialog(BaseDialog):
    ...
    def _render_body(self):
        ...
        with self.create_actions_row():
            ...
            ui.button('Approve & Create Organization', on_click=self._approve).classes('btn').props('color=positive')
    ...
    async def _approve(self):
        ...
        try:
            ...
            ui.notify(f'Organization "{self.request.name}" created successfully', type='positive')
            await self.close()
            if self.on_save:
                await self.on_save()

        except Exception as e:
            logger.error("Failed to approve organization request %s: %s", self.request.id, e, exc_info=True)
