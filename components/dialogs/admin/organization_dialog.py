...
class OrganizationDialog(BaseDialog):
    ...
    async def show(self) -> None:
        ...
        title = (
            ...
            else f"Edit {self.organization.name}"
        )
    ...
    def _render_body(self) -> None:
        ...
        with self.create_form_grid(columns=2):
            with ui.element("div"):
                self.name_input = ui.input(
                    label="Name",
                    value=self.organization.name if self.organization else "",
                ).classes("w-full")
            with ui.element("div"):
                self.active_toggle = ui.checkbox(
                    text="Active",
                    value=(
                        True
                        if self.organization is None
                        else self.organization.is_active
                    ),
                )
        with ui.element("div").classes("mt-1"):
            ...
            self.desc_input = (
                ui.input(
                    label="Description",
                    value=self.organization.description if self.organization else "",
                )
        ...
        with self.create_actions_row():
            ...
            ui.button(
                "Save" if self.organization else "Create", on_click=self._save
    ...
    async def _save(self) -> None:
        ...
        description = (
            (self.desc_input.value if self.desc_input else "").strip()
            if self.desc_input
            else None
        )
