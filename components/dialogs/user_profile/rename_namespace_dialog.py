...
class RenameNamespaceDialog(BaseDialog):
    ...
    def _render_body(self):
        ...
        ui.label(f'Current name: {self.namespace.display_name}').classes('text-sm text-secondary mb-4')

        # Input fields
        with self.create_form_grid(columns=1):
            with ui.element('div'):
                self.display_name_input = ui.input(
                    label='Display Name',
                    value=self.namespace.display_name
        ...
        self.visibility_checkbox = ui.checkbox(
            ...
            value=self.namespace.is_public
        ).classes('mt-4')
    ...
    async def _save(self):
        ...
        try:
            ...
            logger.error("Failed to update namespace %s: %s", self.namespace.id, e, exc_info=True)
