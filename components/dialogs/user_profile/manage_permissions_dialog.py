...
class ManagePermissionsDialog(BaseDialog):
    ...
    async def show(self):
        ...
        self.permissions = (
            await PresetNamespacePermission.filter(namespace_id=self.namespace.id)
    ...
    def _render_body(self):
        ...
        ui.label(
            "Grant other users permission to create, update, or delete presets in your namespace."
        ).classes("text-sm text-secondary mb-2")

        # Add user button
        ui.button(
            "Add User", icon="person_add", on_click=self._show_add_permission_dialog
            ...
            with ui.element("div").classes("card-body"):
                with ui.row().classes("items-center justify-between w-full"):
                    with ui.column().classes("gap-1"):
                        ...
                        ui.label(
                            ", ".join(perms_list) if perms_list else "No permissions"
                    ...
                    with ui.row().classes("gap-1"):
                        ...
                        ui.button(icon="edit", on_click=edit_perm).classes(
                            "btn btn-sm"
                        ).props("flat color=primary").tooltip("Edit Permissions")
                        ...
                        ui.button(icon="delete", on_click=remove_perm).classes(
                            "btn btn-sm"
                        ).props("flat color=negative").tooltip("Remove User")

    async def _show_add_permission_dialog(self):
        """Show dialog to add a new user permission."""
        add_dialog = AddPermissionDialog(
            namespace=self.namespace, on_save=self._refresh_permissions
        )
    ...
    async def _show_edit_permission_dialog(self, permission: PresetNamespacePermission):
        ...
        edit_dialog = EditPermissionDialog(
            permission=permission, on_save=self._refresh_permissions
        )
