...
class CreateApiKeyDialog(BaseDialog):
    ...
    def _render_create_form(self) -> None:
        ...
        ui.label("Create a new API key for accessing the SahaBot API.").classes(
            "text-secondary mb-2"
        )
        ...
        with self.create_form_grid(columns=1):
            with ui.element("div"):
                self.name_input = ui.input(
                    label="Key Name", placeholder='e.g., "Production Bot"'
                ).classes("w-full")
        ...
        with self.create_actions_row():
            ...
            ui.button("Generate", on_click=self._handle_create).classes("btn").props(
                "color=positive"
            )
    ...
    def _render_token_display(self) -> None:
        ...
        with ui.row().classes("w-full items-center gap-2"):
            ui.input(value=self.generated_token).props("readonly").classes(
            ...
            ui.button(icon="content_copy", on_click=copy_token).props("flat").classes(
                "btn"
            )

        # Close button
        with ui.row().classes("w-full justify-end mt-4"):
            ui.button("Close", on_click=self._handle_close).classes("btn").props(
                "color=primary"
            )
...
class DisplayTokenDialog(BaseDialog):
    ...
    def _render_body(self) -> None:
        ...
        # Token display with copy button
        with ui.row().classes("w-full items-center gap-2"):
            ui.input(value=self.token).props("readonly").classes(
                "flex-1 font-mono text-sm"
            )
        ...
        # Close button
        with ui.row().classes("w-full justify-end mt-4"):
            ui.button("Close", on_click=self.close).classes("btn").props(
                "color=primary"
            )
