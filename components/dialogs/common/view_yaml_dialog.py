...
class ViewYamlDialog(BaseDialog):
    ...
    async def show(self):
        ...
        self.create_dialog(
            title=f'Preset: {self.preset.name}',
            icon='visibility',
            max_width='800px'
        )
        await super().show()
    ...
    def _render_body(self):
        ...
        with ui.column().classes('gap-2 mb-4'):
            ...
            if self.preset.description:
                with ui.row().classes('items-center gap-2'):
                    ...
                    ui.label(f'Description: {self.preset.description}').classes('text-sm')

            # Scope
            with ui.row().classes('items-center gap-2'):
                if self.preset.namespace:
                    ui.icon('folder', size='sm')
                    ui.label(f'Namespace: {self.preset.namespace.display_name}').classes('text-sm')
                else:
            ...
            if self.preset.user:
                with ui.row().classes('items-center gap-2'):
                    ...
                    ui.label(f'Created by: {self.preset.user.get_display_name()}').classes('text-sm')
        ...
        ui.code(self.yaml_content, language='yaml').classes('w-full').style('max-height: 400px; overflow-y: auto;')
        ...
        with self.create_actions_row():
            ...
            ui.button('Copy YAML', icon='content_copy', on_click=self._copy_yaml).classes('btn').props('color=primary')
