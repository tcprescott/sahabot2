...
class ScheduledTaskDialog(BaseDialog):
    ...
    def _render_body(self):
        ...
        with self.create_form_grid(columns=1):
            ...
            self.task_type_select = (
                ui.select(
                    ...
                    value=(
                        ...
                        if self.task
                        else TaskType.EXAMPLE_LOG.value
                    ),
            ...
            self.schedule_type_select = (
                ui.select(
                    options=schedule_type_options,
                    label="Schedule Type",
                    value=(
                        self.task.schedule_type.value
                        if self.task
                        else ScheduleType.INTERVAL.value
                    ),
                ...
                with ui.row().classes("w-full gap-2"):
                    self.interval_input = (
                        ui.number(
                            label="Interval",
                            value=(
                                self.task.interval_seconds // 60
                                if self.task and self.task.interval_seconds
                                else 5
                            ),
                            min=1,
                            step=1,
            ...
            with self.cron_container:
                self.cron_input = (
                    ui.input(
                        ...
                        value=(
                            ...
                            if self.task and self.task.cron_expression
                            else ""
                        ),
                    )
                    .classes("w-full")
                ...
                ui.label("Format: minute hour day month weekday").classes(
                    "text-xs text-secondary"
                )
                ui.label('Example: "0 9 * * *" = Every day at 9:00 AM UTC').classes(
                    "text-xs text-secondary"
                )
            ...
            with self.config_container:
                ...
                self.config_message_input = (
                    ui.input(
                        label="Custom Message",
                        placeholder="Enter a custom message for the log",
                        value=(
                            self.task.task_config.get("message", "")
                            if self.task and self.task.task_config
                            else ""
                        ),
                    )
                    .classes("w-full")
            ...
            self.is_active_switch = ui.checkbox(
                "Active", value=self.task.is_active if self.task else True
            )
        ...
        with self.create_actions_row():
            ...
            ui.button("Save" if self.task else "Create", on_click=self._save).classes(
                "btn"
        ...
        if schedule_type == ScheduleType.INTERVAL:
            ...
            try:
                scheduled_time = datetime.fromisoformat(
                    self.scheduled_time_input.value
