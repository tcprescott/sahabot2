...
class MemberPermissionsDialog(BaseDialog):
    ...
    def _render_body(self) -> None:
        with self.create_form_grid(columns=1):
            with ui.element('div').classes('span-1'):
                ui.label(f'Select permissions for {self.username}').classes('font-semibold mb-2')
                # Build options dict for multiselect
                options = {ptype['name']: f"{ptype['name']} - {ptype['description']}" for ptype in self.available_types}
                self.permissions_select = ui.select(
                    label='Permissions',
                    options=options,
    ...
    async def _save(self) -> None:
        ...
        await self.service.set_permissions_for_member(self.organization_id, self.user_id, selected)
        if self.on_save:
            await self.on_save()
        await self.close()
