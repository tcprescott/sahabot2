...
class OrganizationInviteDialog(BaseDialog):
    ...
    def __init__(
        ...
        *,
        on_submit: Optional[
            Callable[[str, Optional[int], Optional[datetime]], None]
        ] = None,
    ) -> None:
        super().__init__()
    ...
    async def show(self) -> None:
        """Display the dialog."""
        self.create_dialog(
            title="Create Invite Link", icon="link", max_width="dialog-card"
        )
        await super().show()
    ...
    def _render_body(self) -> None:
        ...
        with self.create_form_grid(columns=1):
            with ui.element("div"):
                self._slug_input = ui.input(
                    label="Invite Link URL", placeholder="my-tournament-2025"
                ).classes("w-full")
                with ui.element("div").classes("text-xs text-secondary mt-1"):
                    ui.label(
                        "Choose a unique URL slug (letters, numbers, hyphens, underscores only)"
                    )
                with ui.element("div").classes("text-xs text-secondary"):
                    ui.label("Full URL will be: /invite/your-slug-here")

            with ui.element("div"):
                self._max_uses_input = ui.input(
            ...
            with ui.element("div"):
                ...
                self._expires_days_input = ui.input(
                    label="Expires in (days)", value="7", placeholder="7"
                ).classes("w-full")
        ...
        with self.create_actions_row():
            ...
            ui.button("Create Invite", on_click=self._handle_submit).classes(
                "btn"
