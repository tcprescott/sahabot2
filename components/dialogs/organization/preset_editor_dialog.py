...
class PresetEditorDialog(BaseDialog):
    ...
    async def show(self):
        ...
        self.create_dialog(
            title=title, icon="edit" if self.is_edit_mode else "add", max_width="900px"
        )
        await super().show()
    ...
    def _render_body(self):
        ...
        with self.create_form_grid(columns=2):
            with ui.element("div"):
                ...
                if self.preset:
                    ...
                    if self.is_edit_mode:
                        self.name_input.props(
                            "readonly"
                        )  # Can't change name in edit mode

            with ui.element("div"):
                self.randomizer_select = (
                    ui.select(
                        label="Randomizer",
                ...
                if self.is_edit_mode:
                    self.randomizer_select.props(
                        "readonly"
                    )  # Can't change randomizer in edit mode
        ...
        ui.label("Edit the preset settings in YAML format below:").classes(
        ...
        self.yaml_editor = (
            ui.textarea(label="YAML Content", value=default_yaml)
        ...
        with ui.row().classes("mt-2"):
            ui.button(
                "Validate YAML", on_click=self._validate_yaml, icon="check_circle"
...
"""
    ...
    async def _validate_yaml(self):
        ...
        try:
            ...
            with self.validation_message:
                with ui.row().classes(
                    "items-center gap-2 p-3 rounded bg-positive text-white"
                ):
    ...
    async def _save(self):
        ...
        try:
            ...
            if self.is_edit_mode:
                ...
                ui.notify(
                    f'{scope.title()} "{name}" created successfully', type="positive"
                )
