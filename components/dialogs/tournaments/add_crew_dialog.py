...
class AddCrewDialog(BaseDialog):
    ...
    async def show(self) -> None:
        ...
        self.create_dialog(
            title=f'Add Crew to {match_title}',
            icon='people',
    ...
    async def _handle_add_crew(self) -> None:
        ...
        if not self.role_input or not self.role_input.value or not self.role_input.value.strip():
        ...
        try:
            ...
            if result:
                ...
                ui.notify('Failed to add crew. Check permissions or if user is already assigned to this role.', type='negative')
