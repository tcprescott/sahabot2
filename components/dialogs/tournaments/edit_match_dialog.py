...
class EditMatchDialog(BaseDialog):
    ...
    def _render_body(self) -> None:
        ...
        with self.create_form_grid(columns=1):
            with ui.element('div'):
                self._title_input = ui.input(
                    label='Match Title',
                    value=self.match.title or ""
                ).classes('w-full')
            
            with ui.element('div'):
                self._scheduled_input = ui.input(
                    label='Scheduled Time',
                    value=local_scheduled
                ).props('type=datetime-local').classes('w-full')
                ui.label('Time will be displayed in your local timezone').classes('text-xs text-secondary')
        ...
        async def load_streams():
            try:
                ...
                if self._stream_select:
                    ...
                    self._stream_select.value = self.match.stream_channel_id if self.match.stream_channel_id else None
                    self._stream_select.update()
                    
                logger.info("Loaded %d stream channels for org %s", len(streams), self.organization_id)
        ...
        with self.create_actions_row():
            ...
            ui.button('Save', on_click=self._handle_save).classes('btn').props('color=positive')
    ...
    async def _handle_save(self) -> None:
        ...
        scheduled_str = self._scheduled_input.value.strip() if self._scheduled_input.value else None
        ...
        if scheduled_str:
            try:
                ...
                logger.info("Parsed scheduled time: %s (will be saved as server timezone)", scheduled_at)
