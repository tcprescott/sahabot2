...
class RegisterPlayerDialog(BaseDialog):
    ...
    def _render_body(self) -> None:
        ...
        with self.create_form_grid(columns=1):
            with ui.element("div"):
                ui.label("Select a user to register for this tournament").classes(
                    "font-semibold mb-2"
                )
                ...
                ui.label("Only members of this organization will be shown").classes(
                    "text-sm text-secondary mt-1"
                )

        with self.create_actions_row():
            ui.button("Cancel", on_click=self.close).classes("btn")
            ui.button("Register Player", on_click=self._handle_register).classes(
                "btn"
    ...
    async def _handle_register(self) -> None:
        ...
        try:
            ...
            if result:
                ...
                ui.notify(
                    "Failed to register player. Check permissions or if user is already registered.",
                    type="negative",
                )
