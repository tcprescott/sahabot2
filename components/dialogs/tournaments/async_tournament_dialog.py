...
class AsyncTournamentDialog(BaseDialog):
    ...
    async def show(self):
        """Display the dialog."""
        title = f'Edit Async Tournament: {self.tournament.name}' if self.tournament else 'Create New Async Tournament'
        self.create_dialog(
            title=title,
    ...
    def _render_body(self):
        ...
        with self.create_form_grid(columns=2):
            with ui.element('div'):
                ...
                ui.label('Allow players to start new races').classes('text-sm text-secondary')

            with ui.element('div'):
                self.runs_per_pool_input = ui.number(
                    label='Runs Per Pool',
        ...
        with self.create_form_grid(columns=1):
            with ui.element('div'):
                if self.discord_channels:
                    ...
                    channel_options.update({ch_id: ch_name for ch_id, ch_name in self.discord_channels})
                    
                    current_value = self.tournament.discord_channel_id if self.tournament else None
                    
                    self.discord_channel_select = ui.select(
                        label='Discord Channel (optional)',
                        options=channel_options,
                        value=current_value
                    ).classes('w-full').props('outlined')
                    ui.label('The Discord channel where players can start async runs').classes('text-sm text-secondary')
                    
                    # Show manual entry option only if enabled
                    if self.allow_manual_channel_id:
                        with ui.element('div').classes('mt-3'):
                            ui.label('Or enter Channel ID manually:').classes('text-sm font-semibold')
                            self.manual_channel_input = ui.input(
                                label='Discord Channel ID',
                                placeholder='e.g., 1234567890123456789',
                                value=str(self.tournament.discord_channel_id) if self.tournament and self.tournament.discord_channel_id else ''
                            ).classes('w-full').props('outlined')
                            ui.label('Manual entry overrides the dropdown selection above').classes('text-sm text-secondary')
                else:
                    # No channels available
                    if self.allow_manual_channel_id:
                        # Allow manual entry if enabled
                        ui.label('No Discord channels found').classes('text-warning')
                        ui.label('Enter the Discord Channel ID manually, or link a Discord server first').classes('text-sm text-secondary mb-2')
                        ...
                        ui.label('Right-click a channel in Discord > Copy Channel ID').classes('text-sm text-secondary')
                    else:
                        # No channels and manual entry disabled
                        ui.label('No Discord channels available').classes('text-warning')
                        ui.label('Link a Discord server in the Discord Servers tab to enable channel selection').classes('text-sm text-secondary')
                        self.discord_channel_select = None
        ...
        with self.create_actions_row():
            ...
            ui.button(
                'Save' if self.tournament else 'Create',
                on_click=self._save
    ...
    async def _save(self):
        ...
        if not self.name_input or not self.name_input.value or not self.name_input.value.strip():
        ...
        if self.manual_channel_input and self.manual_channel_input.value and self.manual_channel_input.value.strip():
