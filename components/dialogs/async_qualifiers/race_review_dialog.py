...
class RaceReviewDialog(BaseDialog):
    ...
    def __init__(
        self, race_data: dict, organization_id: int, on_save: Optional[Callable] = None
    ):
        """
    ...
    def _render_body(self):
        ...
        with ui.column().classes("gap-sm w-full"):
            ...
            self.create_info_row(
                "Finish Time", self.race_data["elapsed_time_formatted"]
            )
            ...
            if self.race_data.get("runner_notes"):
                ...
                with ui.card().classes("w-full p-2 bg-gray-100"):
                    ui.label(self.race_data["runner_notes"]).classes(
                        "text-sm whitespace-pre-wrap"
                    )

            # Show review flag if present
            if self.race_data.get("review_requested_by_user"):
                ui.label("⚠️ FLAGGED FOR REVIEW BY USER").classes(
                    "font-bold text-sm mt-4 text-warning"
                )
                if self.race_data.get("review_request_reason"):
                    ui.label("Reason for Review Request:").classes(
                        "font-semibold text-sm mt-2"
                    )
                    with ui.card().classes(
                        "w-full p-2 bg-yellow-50 border-2 border-yellow-400"
                    ):
                        ui.label(self.race_data["review_request_reason"]).classes(
        ...
        with ui.column().classes("gap-md w-full"):
            ...
            ui.label("Elapsed Time Override (HH:MM:SS) - Optional").classes(
                "font-semibold text-sm"
            )
            ui.label("Only change this if correcting an incorrect time").classes(
                "text-xs text-secondary"
            )
            self.elapsed_time_input = ui.input(
                placeholder="HH:MM:SS (e.g., 01:23:45)",
                validation={
                    "Invalid format": lambda v: not v or self._validate_time_format(v)
                },
            ).classes("w-full")

            # Reviewer Notes
            ui.label("Reviewer Notes").classes("font-semibold text-sm")
            self.reviewer_notes_input = (
                ui.textarea(
            ...
            if self.race_data.get("reviewed_by"):
                with ui.card().classes("w-full p-2 bg-blue-50 mt-2"):
                    ui.label(
                        f"Previously reviewed by {self.race_data['reviewed_by']['discord_username']}"
                    ).classes("text-sm font-semibold")
                    if self.race_data.get("reviewed_at"):
                        ui.label(f"at {self.race_data['reviewed_at']}").classes(
                            "text-xs text-secondary"
                        )
        ...
        with self.create_actions_row():
            ...
            ui.button("Save Review", on_click=self._save_review).classes("btn").props(
                "color=positive"
            )
