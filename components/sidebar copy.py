...
class Sidebar:
    ...
    def __init__(self, tabs: list, panels: Optional[ui.tabs] = None):
        ...
        self.tabs = tabs
        self.panels = panels
        # On mobile (detected via user agent or default), start collapsed (hidden)
        # On desktop, use stored preference (default to not collapsed)
        # For simplicity, we'll start collapsed on first visit, user can toggle
        self.collapsed: bool = bool(app.storage.user.get('sidebar_collapsed', True))
    ...
    def _toggle_drawer(self) -> None:
        ...
        if self._sidebar_element and self._content_element:
            ...
            if self.collapsed:
                self._sidebar_element.classes(add='collapsed')
            else:
                self._sidebar_element.classes(remove='collapsed')
            
            # Toggle content classes
            if self.collapsed:
                self._content_element.classes(remove='content-sidebar-expanded', add='content-sidebar-collapsed')
            else:
                self._content_element.classes(remove='content-sidebar-collapsed', add='content-sidebar-expanded')
            
            # Toggle backdrop on mobile
            if self._backdrop_element:
                if self.collapsed:
                    self._backdrop_element.classes(remove='active')
                else:
                    self._backdrop_element.classes(add='active')
