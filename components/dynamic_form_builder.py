...
class DynamicFormBuilder:
    ...
    def render(self, existing_values: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not self.schema or 'fields' not in self.schema:
            ...
            ui.label('No form configuration defined. Please enter settings as JSON:').classes('font-bold mb-2')
            self.fields['_json_fallback'] = ui.textarea(
                label='Settings JSON',
        ...
        for field_config in fields_config:
            ...
            # Get pre-fill value
            default_value = existing_values.get(field_name) if existing_values else field_config.get('default')
            ...
            if field_type == 'select':
                ...
                self._render_number(field_name, label_text, default_value, min_val, max_val)
            elif field_type == 'checkbox':
                self._render_checkbox(field_name, label_text, default_value)
            elif field_type == 'textarea':
                placeholder = field_config.get('placeholder', '')
                self._render_textarea(field_name, label_text, default_value, placeholder)
            else:  # text or default
                placeholder = field_config.get('placeholder', '')
                self._render_text(field_name, label_text, default_value, placeholder)
    
    def _render_select(self, name: str, label: str, options: List[str], default: Optional[str]) -> None:
    ...
    def _render_number(self, name: str, label: str, default: Optional[float], min_val: Optional[float], max_val: Optional[float]) -> None:
        """Render number input."""
        with ui.column().classes('w-full mb-4'):
            ui.label(label).classes('font-bold mb-1')
            kwargs = {}
    ...
    def _render_checkbox(self, name: str, label: str, default: Optional[bool]) -> None:
        ...
        with ui.row().classes('items-center mb-4'):
            self.fields[name] = ui.checkbox(label, value=default if default is not None else False)
    
    def _render_text(self, name: str, label: str, default: Optional[str], placeholder: str) -> None:
        """Render text input."""
        with ui.column().classes('w-full mb-4'):
            ui.label(label).classes('font-bold mb-1')
    ...
    def _render_textarea(self, name: str, label: str, default: Optional[str], placeholder: str) -> None:
        """Render textarea."""
        with ui.column().classes('w-full mb-4'):
            ui.label(label).classes('font-bold mb-1')
            self.fields[name] = ui.textarea(
                label='',
                placeholder=placeholder,
                value=default or ''
