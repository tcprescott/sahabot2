...
def check_discord_config():
    ...
    # Check Client ID
    print(f"✓ DISCORD_CLIENT_ID: {settings.DISCORD_CLIENT_ID[:10]}..." if len(settings.DISCORD_CLIENT_ID) > 10 else f"✗ DISCORD_CLIENT_ID: {settings.DISCORD_CLIENT_ID}")
    ...
    # Check Client Secret
    has_secret = settings.DISCORD_CLIENT_SECRET and settings.DISCORD_CLIENT_SECRET != "your_discord_client_secret"
    print(f"✓ DISCORD_CLIENT_SECRET: {'*' * 10}..." if has_secret else "✗ DISCORD_CLIENT_SECRET: NOT SET")
    if not has_secret:
    ...
    elif "localhost" in settings.DISCORD_REDIRECT_URI and settings.ENVIRONMENT == "production":
    ...
    # Check Bot Token
    has_bot_token = settings.DISCORD_BOT_TOKEN and settings.DISCORD_BOT_TOKEN != "your_discord_bot_token_here"
    print(f"✓ DISCORD_BOT_TOKEN: {'*' * 10}..." if has_bot_token else "⚠ DISCORD_BOT_TOKEN: NOT SET")
    if not has_bot_token:
        warnings.append("DISCORD_BOT_TOKEN is not set (needed for Discord bot features)")
