...
async def generate_mock_data(preset: str = "tiny"):
    ...
    try:
        ...
        logger.info("Generating data: %d users, %d orgs, %d tournaments...", 
                   config['num_users'], config['num_orgs'], config['num_tournaments'])
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Set up test environment for SahaBot2"
    )
    parser.add_argument(
        '--skip-data',
        action='store_true',
