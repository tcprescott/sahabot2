...
def register():
    ...
    async def login_page(redirect: str = None):
        ...
        app.storage.browser["oauth_states"] = {
            k: v
            for k, v in app.storage.browser["oauth_states"].items()
    ...
    async def callback_page(code: str = None, state: str = None, error: str = None):
        ...
        async def content(
            page: BasePage,
        ):  # noqa: ARG001 - required by BasePage pattern
            ...
            if error:
                with ui.element("div").classes("card text-center"):
                    ...
                    with ui.element("div").classes("card-body"):
                        ...
                        ui.button(
                            "Try Again", on_click=lambda: ui.navigate.to("/auth/login")
            ...
            logger.info(
                "OAuth callback - state: %s, stored states count: %d",
            ...
            if not state:
                ...
                with ui.element("div").classes("card text-center"):
                    with ui.element("div").classes("card-header"):
                        ui.label("Authentication Failed")
                    with ui.element("div").classes("card-body"):
                        ui.label("Invalid state token. Please try again.").classes(
                            "text-error"
                        )
                        ui.button(
                            "Try Again", on_click=lambda: ui.navigate.to("/auth/login")
            ...
            if state not in stored_states:
                logger.warning(
                    "OAuth state mismatch - state not found in stored states. Received: %s",
                    state[:10] + "...",
                )
                logger.debug(
                    "Stored states: %s", [k[:10] + "..." for k in stored_states.keys()]
                )
                with ui.element("div").classes("card text-center"):
                    with ui.element("div").classes("card-header"):
                        ui.label("Authentication Failed")
                    with ui.element("div").classes("card-body"):
                        ...
                        ui.label(
                            "This can happen if you took too long to log in or if cookies are disabled."
            ...
            if time.time() - state_timestamp > 600:
                ...
                with ui.element("div").classes("card text-center"):
                    with ui.element("div").classes("card-header"):
                        ui.label("Authentication Failed")
                    with ui.element("div").classes("card-body"):
                        ui.label("Login session expired. Please try again.").classes(
                            "text-error"
                        )
                        ui.button(
                            "Try Again", on_click=lambda: ui.navigate.to("/auth/login")
            ...
            if code:
                try:
                    ...
                    # Authenticate user
                    logger.info(
                        "Starting OAuth authentication for code: %s...",
                    ...
                    logger.info(
                        "OAuth authentication successful, redirecting to: %s",
                        redirect_url,
                    )
                    ...
                    logger.error(
                        "OAuth authentication failed: %s", str(e), exc_info=True
                    )
                ...
                with ui.element("div").classes("card text-center"):
                    with ui.element("div").classes("card-header"):
                        ui.label("Authentication Failed")
                    with ui.element("div").classes("card-body"):
                        ui.label("No authorization code received.").classes(
                            "text-error"
                        )
                        ui.button(
                            "Try Again", on_click=lambda: ui.navigate.to("/auth/login")
