        ...
        async def content(page: BasePage):
            ...
            if not invite:
                with Card.create(title="Invalid Invite"):
                    ...
                    ui.button(
                        "Go to Home", on_click=lambda: ui.navigate.to("/")
                    ).classes("btn mt-2")
                return
                ...
                if not page.user:
                    ...
                    Card.action(
                        ...
                        on_click=lambda: ui.navigate.to(
                            f"/auth/login?redirect=/invite/{slug}"
                        ),
                    )
                elif not can_use:
                    ...
                    Card.simple(
                        ...
                        or "You have been invited to join this organization.",
                    )

                    async def accept_invite():
                        success, error = await invite_service.use_invite(
                            slug, page.user.id
                        )
                        if success:
                            ui.notify(
                                "Successfully joined the organization!", type="positive"
                            )
                            ui.navigate.to(f"/org/{org.id}")
                        else:
                            ui.notify(
                                error or "Failed to join organization", type="negative"
                            )
                    ...
                    with Card.create(title="Accept Invitation"):
                        ...
                        with ui.row().classes("gap-2 mt-2"):
                            ui.button(
                                "Accept & Join", icon="check", on_click=accept_invite
