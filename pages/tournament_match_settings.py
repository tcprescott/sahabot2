...
def register():
    ...
    async def match_settings_submission_page(match_id: int):
        ...
        async def content(page: BasePage):
            ...
            # page.user is guaranteed by authenticated_page()
            ...
            existing_submission = await service.get_submission(
                page.user, match_id, game_number=1
            )
                    ...
                    if match.scheduled_at:
                        with ui.row().classes("items-center gap-2"):
                            ...
                            DateTimeLabel.create(
                                match.scheduled_at, format_type="datetime"
                            )
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    with ui.row().classes("items-center gap-4 mb-4"):
                        ...
                        game_number_input = ui.number(
                            label="", value=1, min=1, max=10, step=1, format="%.0f"
                        ).classes("w-32")

                    # Dynamic form based on tournament configuration
                    form_builder = DynamicFormBuilder(
                        match.tournament.settings_form_schema
                    )
                    existing_values = (
                        existing_submission.settings if existing_submission else None
                    )
                    ...
                    with ui.row().classes("mt-4 gap-4"):
                        ...
                        async def submit_settings():
                            ...
                            try:
                                ...
                                if page.user is None:
                                    ui.notify(
                                        "User session expired", color="negative"
                                    )
                                    return
                                ...
                                if submission:
                                    ...
                                    ui.notify(
                                        "Failed to submit settings", color="negative"
                                    )
                        ...
                        ui.button("Submit Settings", on_click=submit_settings).props(
                            "color=primary"
                        )
                ...
                with ui.element("div").classes("card"):
                    ...
                    with ui.element("div").classes("card-body"):
                        with ui.row().classes("items-center gap-2 mb-2"):
                            ui.label("Submitted:")
                            DateTimeLabel.create(
                                existing_submission.submitted_at, format_type="relative"
                            )
