        ...
        async def content(page: BasePage):
                ...
                with ui.element("div").classes("card-body"):
                    with ui.row().classes("items-center gap-md"):
                        ...
                        with ui.column().classes("gap-sm"):
                            ui.label("Test Page - DEBUG Mode Only").classes(
                                "text-xl font-bold"
                            )
                            ui.label(
                                "This page displays examples of all components and UI elements for CSS testing."
                            )
            ...
            with ui.element("div").classes("card mt-4"):
                ...
                with ui.element("div").classes("card-body"):
                    with ui.row().classes("gap-md flex-wrap"):
                        ...
                        ui.button(
                            "Primary Button",
                            on_click=lambda: ui.notify("Primary clicked!"),
                        ).classes("btn btn-primary")
                        ui.button(
                            "Secondary Button",
                            on_click=lambda: ui.notify("Secondary clicked!"),
                        ).classes("btn btn-secondary")
                        ui.button(
                            "Danger Button",
                            on_click=lambda: ui.notify("Danger clicked!"),
                        ).classes("btn btn-danger")
                        ui.button(
                            "Success Button",
                            on_click=lambda: ui.notify("Success clicked!"),
                        ).classes("btn btn-success")
                        ui.button(
                            "Warning Button",
                            on_click=lambda: ui.notify("Warning clicked!"),
                        ).classes("btn btn-warning")
                    with ui.row().classes("gap-md flex-wrap mt-4"):
                        ui.button(
                            "Disabled Button",
                            on_click=lambda: ui.notify("Should not see this"),
                        ).props("disabled").classes("btn")
                        ui.button(
                            "Icon Button",
                            icon="star",
                            on_click=lambda: ui.notify("Icon button!"),
                        ).classes("btn btn-primary")
                        ui.button(
                            "Loading Button", on_click=lambda: ui.notify("Loading...")
                        ).props("loading").classes("btn")
                            ...
                            async def show_example_dialog():
                                class ExampleDialog(BaseDialog):
                                    def _render_body(self):
                                        ui.label(
                                            "This is an example dialog using BaseDialog."
                                        )
                                        ui.label(
                                            "It demonstrates the standard dialog structure."
                                        ).classes("text-secondary mt-2")
                                        ui.separator().classes("my-4")
                                        with self.create_actions_row():
                                            ui.button(
                                                "Cancel", on_click=self.close
                                            ).classes("btn")
                                            ui.button(
                                                "OK", on_click=self.close
                                            ).classes("btn").props("color=positive")

                                dialog = ExampleDialog()
                                dialog.create_dialog(
                                    title="Example Dialog", icon="info"
                                )
                                await dialog.show()
            ...
            with ui.element("div").classes("card mt-4"):
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    ui.input(
                        label="Username", placeholder="Enter username", value=""
                    ).classes("w-full")
            ...
            with ui.element("div").classes("card mt-4"):
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    columns = [
                        ...
                        TableColumn(
                            label="Status",
                            cell_render=lambda row: ui.badge(row["status"]).classes(
                                "badge-success"
                ...
                with ui.element("div").classes("card-body"):
                    with ui.row().classes("gap-md flex-wrap"):
                        ui.button(
                            ...
                            on_click=lambda: ui.notify("Success!", type="positive"),
                        ).classes("btn btn-success")
                        ui.button(
                            "Error Notification",
                            on_click=lambda: ui.notify("Error!", type="negative"),
                        ).classes("btn btn-danger")
                        ui.button(
                            "Warning Notification",
                            on_click=lambda: ui.notify("Warning!", type="warning"),
                        ).classes("btn btn-warning")
                        ui.button(
                            "Info Notification",
                            on_click=lambda: ui.notify("Info!", type="info"),
                        ).classes("btn")

            # DynamicFormBuilder Component
            with ui.element("div").classes("card mt-4"):
                with ui.element("div").classes("card-header"):
                    ui.label("DynamicFormBuilder Component").classes(
                        "text-2xl font-bold"
                    )
                with ui.element("div").classes("card-body"):
                    ui.label("Example of a dynamic form based on JSON schema:").classes(
                        "mb-2"
                    )
                    schema = {
                        "fields": [
            ...
            with ui.element("div").classes("card mt-4"):
                ...
                with ui.element("div").classes("card-body"):
                    ...
                    ui.link(
                        "External Link (GitHub)", "https://github.com", new_tab=True
                    ).classes("mt-2")
                    with ui.link(target="https://nicegui.io", new_tab=True).classes(
                        "mt-2"
                    ):
            ...
            with ui.element("div").classes("card mt-4"):
                with ui.element("div").classes("card-header"):
                    ui.label("Spinners & Loading Indicators").classes(
                        "text-2xl font-bold"
                    )
                with ui.element("div").classes("card-body"):
                    with ui.row().classes("gap-md flex-wrap items-center"):
            ...
            with ui.element("div").classes("card mt-4 mb-4"):
                ...
                with ui.element("div").classes("card-body"):
                    with ui.row().classes("gap-md"):
                        ui.button("Hover for tooltip", on_click=lambda: None).tooltip(
                            "This is a tooltip"
                        ).classes("btn")
                        ui.label("Hover over this text").tooltip("Tooltip on text")
                        ui.icon("info").tooltip("Tooltip on icon").classes(
                            "cursor-pointer"
                        )
