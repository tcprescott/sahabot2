...
def register():
    ...
    async def initiate_link():
        ...
        app.storage.browser["racetime_oauth_states"] = {
            k: v
            for k, v in app.storage.browser["racetime_oauth_states"].items()
    ...
    async def link_callback(code: str = None, state: str = None, error: str = None):
        ...
        async def content(
            ...
            if not state:
                logger.warning(
                    "RaceTime OAuth callback received without state parameter"
                )
                with ui.element("div").classes("card text-center"):
                    with ui.element("div").classes("card-header"):
            ...
            if state not in stored_states:
                ...
                with ui.element("div").classes("card text-center"):
                    ...
                    with ui.element("div").classes("card-body"):
                        ...
                        ui.label(
                            "This can happen if you took too long or if cookies are disabled."
            ...
            if time.time() - state_data["timestamp"] > 600:
                ...
                with ui.element("div").classes("card text-center"):
                    with ui.element("div").classes("card-header"):
                        ui.label("Linking Failed")
                    with ui.element("div").classes("card-body"):
                        ui.label("Linking session expired. Please try again.").classes(
                            "text-error"
                        )
                        ui.button(
                            "Back to Profile",
            ...
            if code:
                try:
                    ...
                    if not current_user:
                        logger.warning(
                            "User %s not found for RaceTime callback", user_id
                        )
                        with ui.element("div").classes("card text-center"):
                            with ui.element("div").classes("card-header"):
                                ui.label("Linking Failed")
                    ...
                    logger.info(
                        "Successfully linked RaceTime account %s to user %s",
                        racetime_id,
                        current_user.id,
                    )
                    ...
                    with ui.element("div").classes("card text-center"):
                        ...
                        with ui.element("div").classes("card-body"):
                            ui.label(
                                f'Your RaceTime.gg account "{racetime_name}" has been linked.'
                    ...
                    logger.error(
                        "Error in RaceTime callback: %s", str(e), exc_info=True
                    )
                    with ui.element("div").classes("card text-center"):
                        with ui.element("div").classes("card-header"):
                            ui.label("Linking Failed")
