...
class RacetimeRoom(Model):
    ...
    slug = fields.CharField(
        max_length=255,
        unique=True,
        description="Full room slug (e.g., 'alttpr/cool-doge-1234')",
    )
    category = fields.CharField(
        max_length=50, description="RaceTime category (e.g., 'alttpr')"
    )
    room_name = fields.CharField(
        max_length=255, description="Room name only (e.g., 'cool-doge-1234')"
    )
    ...
    status = fields.CharField(
        max_length=50,
        null=True,
        description="Current race status (open, pending, in_progress, finished, cancelled)",
    )
    ...
    created_at = fields.DatetimeField(
        auto_now_add=True, description="When room was created or joined"
    )
