...
class RacetimeBot(Model):
    ...
    category = fields.CharField(
        max_length=50,
        unique=True,
        description="RaceTime category slug (e.g., 'alttpr', 'smz3')",
    )
    ...
    is_active = fields.BooleanField(
        default=True, description="Whether this bot is enabled"
    )
    ...
    status = fields.IntEnumField(
        BotStatus, default=BotStatus.UNKNOWN, description="Current connection status"
    )
    status_message = fields.TextField(
        null=True, description="Additional status information or error message"
    )
    last_connected_at = fields.DatetimeField(
        null=True, description="Timestamp of last successful connection"
    )
    last_status_check_at = fields.DatetimeField(
        null=True, description="Timestamp of last status check"
    )
...
class RacetimeBotOrganization(Model):
    ...
    bot = fields.ForeignKeyField(
        "models.RacetimeBot", related_name="organization_assignments"
    )
    organization = fields.ForeignKeyField(
        "models.Organization", related_name="racetime_bots"
    )
    is_active = fields.BooleanField(
