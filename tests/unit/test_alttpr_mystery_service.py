...
class TestALTTPRMysteryService:
    ...
    def basic_mystery_weights(self):
        ...
        return {"weights": {"open": 10, "standard": 5, "inverted": 3}}
    ...
    def test_validate_mystery_weights_with_subweights(
        self, service, mystery_weights_with_subweights
    ):
        """Test validation of mystery weights with subweights."""
        is_valid, error = service.validate_mystery_weights(
            mystery_weights_with_subweights
        )
        assert is_valid is True
        assert error is None

    def test_validate_mystery_weights_with_entrance(
        self, service, mystery_weights_with_entrance
    ):
        """Test validation of mystery weights with entrance shuffle."""
        is_valid, error = service.validate_mystery_weights(
            mystery_weights_with_entrance
        )
        assert is_valid is True
        assert error is None

    def test_validate_mystery_weights_with_customizer(
        self, service, mystery_weights_with_customizer
    ):
        """Test validation of mystery weights with customizer."""
        is_valid, error = service.validate_mystery_weights(
            mystery_weights_with_customizer
        )
        assert is_valid is True
        assert error is None
    ...
    def test_roll_mystery_settings_with_subweights(
        self, service, mystery_weights_with_subweights
    ):
        """Test rolling mystery settings with subweights."""
        # Roll multiple times to check consistency
        for _ in range(5):
            settings, description = service._roll_mystery_settings(
                mystery_weights_with_subweights
            )
    ...
    def test_roll_mystery_settings_with_entrance(
        self, service, mystery_weights_with_entrance
    ):
    ...
    def test_roll_mystery_settings_with_customizer(
        self, service, mystery_weights_with_customizer
    ):
