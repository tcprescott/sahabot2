...
class TestScheduledTaskRepository:
    ...
    async def test_create_interval_task(self, db):
        """Test creating an interval-based scheduled task."""
        repo = ScheduledTaskRepository()

        # Note: Since we're using SQLite for tests and Organization model may not be in test schema,
        # we'll use a mock organization_id. In real usage, the organization_id would be validated.
    ...
    async def test_create_cron_task(self, db):
    ...
    async def test_list_by_org(self, db):
        ...
        org1_tasks = await repo.list_by_org(organization_id=1)
        org2_tasks = await repo.list_by_org(organization_id=2)
    ...
    async def test_list_active_tasks(self, db):
        ...
        active_tasks = await repo.list_by_org(organization_id=1, active_only=True)
