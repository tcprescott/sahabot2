...
class TestAvianartService:
    ...
    def mock_generate_response(self):
        ...
        return {"response": {"hash": "test_hash_12345"}}
    ...
    def mock_permlink_response_generating(self):
        ...
        return {"response": {"status": "generating", "hash": "test_hash_12345"}}

    @pytest.fixture
    def mock_permlink_response_failure(self):
        """Mock response from permlink API call when generation failed."""
        return {
            "response": {"status": "failure", "message": "Invalid preset configuration"}
        }

    @pytest.mark.asyncio
    async def test_generate_successful(
        self, service, mock_generate_response, mock_permlink_response_finished
    ):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            mock_get_response.json = MagicMock(
                return_value=mock_permlink_response_finished
            )
    ...
    async def test_generate_with_failure_status(
        self, service, mock_generate_response, mock_permlink_response_failure
    ):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            mock_get_response.raise_for_status = MagicMock()
            mock_get_response.json = MagicMock(
                return_value=mock_permlink_response_failure
            )
            mock_client.get.return_value = mock_get_response
    ...
    async def test_generate_timeout(
        self, service, mock_generate_response, mock_permlink_response_generating
    ):
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            mock_get_response.raise_for_status = MagicMock()
            mock_get_response.json = MagicMock(
                return_value=mock_permlink_response_generating
            )
            mock_client.get.return_value = mock_get_response
    ...
    def test_extract_file_select_code_basic(self, service):
        ...
        result = {
            "response": {
                "spoiler": {"meta": {"hash": "Bow, Bombs, Hookshot, Mushroom, Lamp"}}
            }
        }
    ...
    def test_extract_file_select_code_mixed(self, service):
        ...
        result = {
            "response": {
                "spoiler": {"meta": {"hash": "Bow, Bomb, Hookshot, Pearl, Key"}}
            }
        }
    ...
    def test_extract_file_select_code_missing_key(self, service):
        ...
        result = {"response": {"spoiler": {}}}  # Missing 'meta' key
    ...
    def test_extract_file_select_code_invalid_structure(self, service):
        ...
        result = {"response": None}  # Invalid structure
