...
class TestAuditService:
    ...
    async def test_log_with_details(self, sample_user):
        ...
        complex_details = {
            "old_value": "foo",
            "new_value": "bar",
            "metadata": {"timestamp": "2025-11-05T12:00:00Z", "changed_by": "admin"},
        }
        mock_audit = AuditLog(
            id=2,
            user_id=sample_user.id,
        ...
        result = await service.log_action(
            user=sample_user, action="update_setting", details=complex_details
        )
    ...
    async def test_log_with_ip_address(self, sample_user):
        ...
        # Mock the repository
        mock_audit = AuditLog(
            id=3, user_id=sample_user.id, action="login", ip_address="192.168.1.100"
        )
        service.audit_repository.create = AsyncMock(return_value=mock_audit)

        # Log action with IP
        result = await service.log_action(
            user=sample_user, action="login", ip_address="192.168.1.100"
        )
    ...
    async def test_log_login(self, sample_user):
        ...
        # Log login
        result = await service.log_login(user=sample_user, ip_address="10.0.0.1")
