...
async def test_advance_match_status(db):
    ...
    tournament = await Tournament.create(
        organization=org, name="Status Test Tournament", is_active=True
    )
    ...
    updated = await service.advance_match_status(
        user=admin_user, organization_id=org.id, match_id=match.id, status="checked_in"
    )
    ...
    # 2. Started
    updated = await service.advance_match_status(
        user=admin_user, organization_id=org.id, match_id=match.id, status="started"
    )
    assert updated is not None
    ...
    # 3. Finished
    updated = await service.advance_match_status(
        user=admin_user, organization_id=org.id, match_id=match.id, status="finished"
    )
    assert updated is not None
    ...
    # 4. Recorded
    updated = await service.advance_match_status(
        user=admin_user, organization_id=org.id, match_id=match.id, status="recorded"
    )
    assert updated is not None
...
async def test_revert_match_status(db):
    ...
    tournament = await Tournament.create(
        organization=org, name="Revert Test Tournament", is_active=True
    )
...
async def test_revert_with_racetime_room_fails(db):
    ...
    tournament = await Tournament.create(
        organization=org, name="Room Test Tournament", is_active=True
    )
...
async def test_sync_racetime_room_status(db):
    ...
    tournament = await Tournament.create(
        organization=org, name="Sync Test Tournament", is_active=True
    )
    ...
    mock_race_data = {"status": {"value": "in_progress"}}
    ...
    with patch("aiohttp.ClientSession", return_value=mock_session):
        ...
        result = await service.sync_racetime_room_status(
            user=admin_user, organization_id=org.id, match_id=match.id
        )
...
async def test_sync_racetime_cancelled_unlinks_room(db):
    ...
    tournament = await Tournament.create(
        organization=org, name="Cancel Test Tournament", is_active=True
    )
    ...
    mock_race_data = {"status": {"value": "cancelled"}}
