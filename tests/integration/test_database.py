...
class TestDatabaseIntegration:
    ...
    async def test_user_audit_log_relationship(self, sample_user):
        ...
        log1 = await AuditLog.create(
            user=sample_user, action="test_action_1", ip_address="127.0.0.1"
        )
        log2 = await AuditLog.create(
            user=sample_user, action="test_action_2", ip_address="127.0.0.1"
        )

        # Fetch user with prefetched audit logs
        user_with_logs = (
            await User.filter(id=sample_user.id).prefetch_related("audit_logs").first()
        )
        ...
        fetched_log1 = (
            await AuditLog.filter(id=log1.id).prefetch_related("user").first()
        )
    ...
    async def test_cascade_delete_behavior(self, sample_user, sample_organization):
        ...
        audit_log = await AuditLog.create(
            user=sample_user, action="test_action", ip_address="127.0.0.1"
        )
    ...
    async def test_transaction_rollback(self, db):
        ...
        try:
            async with in_transaction():
                # Create valid user
                await User.create(
                    discord_id=999888777, discord_username="transaction_test_user"
                )
    ...
    async def test_concurrent_operations(self, db):
        ...
        async def create_user(discord_id):
            return await User.create(
                discord_id=discord_id, discord_username=f"concurrent_user_{discord_id}"
            )
