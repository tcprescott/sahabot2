...
class TournamentManagementView:
        ...
        if not can_manage:
            with ui.element("div").classes("card"):
                ...
                with ui.element("div").classes("card-body"):
                    ui.label(
                        "You need TOURNAMENT_MANAGER permissions to access this page."
    ...
    async def _render_content(self) -> None:
        ...
        # Get all tournaments
        tournaments = await self.tournament_service.list_all_org_tournaments(
        ...
        with ui.element("div").classes("card"):
            ...
            with ui.element("div").classes("card-body"):
                ui.label(
                    "Manage tournament registrations for your organization."
                ).classes("text-secondary")
            ...
            with ui.element("div").classes("card-body"):
                with ui.row().classes("full-width items-center gap-4"):
                    ...
                    if not tournaments:
                        ...
                        tournament_options = {
                            t.id: f"{t.name} {'(Active)' if t.is_active else '(Inactive)'}"
    ...
    async def _render_registrations(self, tournament_id: int) -> None:
        ...
        with Card.create(title=f"Registered Players ({len(registrations)})"):
            ...
            with ui.row().classes("full-width justify-end mb-4"):
                ui.button(
                    "Register Player", icon="person_add", on_click=open_register_dialog
