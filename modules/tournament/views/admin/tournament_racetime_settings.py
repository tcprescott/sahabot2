...
class TournamentRacetimeSettingsView:
    ...
    async def _load_bots(self):
        ...
        from application.services.racetime.racetime_bot_service import (
            RacetimeBotService,
        )

        bot_service = RacetimeBotService()
        self.bots = await bot_service.get_bots_for_organization(
            ...
            with ui.element("div").classes("card-body"):
                if not self.bots:
                    with ui.element("div").classes("text-center p-4 text-secondary"):
                        ...
                        ui.label(
                            "Contact an administrator to assign bots to this organization"
                        ).classes("text-xs")
                else:
                    ui.label(
                        f'{len(self.bots)} bot{"s" if len(self.bots) != 1 else ""} assigned to this organization'
                    ...
                    with ui.row().classes(
                        "items-center gap-2 mt-4 p-3 rounded bg-info-light"
                    ):
                        ui.icon("info", size="sm").classes("text-info")
                        ui.label(
                            "To assign additional bots to this organization, contact a global administrator."
                        ).classes("text-sm text-info")
    ...
    async def _render_room_settings_card(self):
        ...
        with ui.element("div").classes("card"):
            ...
            with ui.element("div").classes("card-body"):
                ui.label(
                    "Configure how RaceTime rooms are created for this tournament"
                ).classes("text-sm text-secondary mb-4")
                ...
                with ui.row().classes("items-center gap-4 mb-4"):
                    ...
                    auto_create_toggle = ui.checkbox(
                        value=self.tournament.racetime_auto_create_rooms or False
                    )
                    ui.label(
                        "Automatically create RaceTime rooms when matches are scheduled"
                ...
                with ui.row().classes("items-center gap-4 mb-4"):
                    ...
                    if not self.bots:
                        ui.label(
                            "No bots available. Contact administrator to assign bots to this organization."
                        ).classes("text-sm text-warning")
                    else:
                        ui.label(
                            "Select which bot will manage race rooms for this tournament"
                        ).classes("text-sm text-secondary")
                ...
                with ui.row().classes("items-center gap-4 mb-4"):
                    ...
                    ui.label("How long before scheduled time to open the room").classes(
                ...
                with ui.row().classes("items-center gap-4 mb-4"):
                    ...
                    require_link_toggle = ui.checkbox(
                        value=self.tournament.require_racetime_link or False
                    )
                    ui.label("Players must have RaceTime accounts linked").classes(
                        "text-sm text-secondary"
                    )
                ...
                ui.label("Race Room Configuration Profile").classes(
                    "text-lg font-bold mt-4 mb-3"
                )
                ui.label(
                    "Select a profile to configure room settings (start delay, time limit, chat permissions, etc.)"
                ).classes("text-sm text-secondary mb-4")
                ...
                with ui.row().classes("items-center gap-4 mb-4"):
                    ...
                    profile_options = {
                        ...
                        **{
                            p.id: f"{p.name}{' (Default)' if p.is_default else ''}"
    ...
    async def _render_profiles_card(self):
        ...
        view = RaceRoomProfileManagementView(
            self.user, self.organization, show_bots=False, on_change=on_profiles_changed
        )
        await view.render()
    ...
    async def _render_profile_preview(self):
        ...
        with self.profile_preview_container:
            if self.selected_profile_id is None:
                ...
                profile = next(
                    (p for p in self.profiles if p.id == self.selected_profile_id), None
                )
                if profile:
                    with ui.element("div").classes("p-4 rounded border"):
                        ui.label(f"Profile: {profile.name}").classes("font-bold mb-2")
                        if profile.description:
                            ui.label(profile.description).classes(
                                "text-sm text-secondary mb-2"
                            )

                        # Display settings in a grid
                        with ui.element("div").classes("grid grid-cols-2 gap-2 mt-3"):
                            ui.label(f"Start delay: {profile.start_delay}s").classes(
                                "text-sm"
                            )
                            ui.label(f"Time limit: {profile.time_limit}h").classes(
                                "text-sm"
                            )
                            ui.label(
                                f'Streaming required: {"Yes" if profile.streaming_required else "No"}'
                            ).classes("text-sm")
                            ui.label(
                                f'Auto-start: {"Yes" if profile.auto_start else "No"}'
                            ).classes("text-sm")
                            ui.label(
                                f'Allow comments: {"Yes" if profile.allow_comments else "No"}'
                            ).classes("text-sm")
                            ui.label(
                                f'Hide comments: {"Yes" if profile.hide_comments else "No"}'
                            ).classes("text-sm")
                            ui.label(
                                f'Pre-race chat: {"Yes" if profile.allow_prerace_chat else "No"}'
                            ).classes("text-sm")
                            ui.label(
                                f'Mid-race chat: {"Yes" if profile.allow_midrace_chat else "No"}'
                            ).classes("text-sm")
                            ui.label(
                                f'Non-entrant chat: {"Yes" if profile.allow_non_entrant_chat else "No"}'
                            ).classes("text-sm")
