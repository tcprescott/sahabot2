...
class StreamChannelService:
    ...
    async def list_org_channels(
        ...
        if not allowed:
            logger.warning(
                "Unauthorized list_org_channels by user %s for org %s",
    ...
    async def create_channel(
        ...
        if not allowed:
            logger.warning(
                "Unauthorized create_channel by user %s for org %s",
                getattr(user, "id", None),
                organization_id,
            )
            return None
    ...
    async def update_channel(
        ...
        *,
        ...
        if not allowed:
            logger.warning(
                "Unauthorized update_channel by user %s for org %s",
                getattr(user, "id", None),
                organization_id,
            )
            return None
    ...
    async def delete_channel(
        self, user: Optional[User], organization_id: int, channel_id: int
    ) -> bool:
        """Delete a stream channel if user can admin the org."""
        allowed = await self.org_service.user_can_admin_org(user, organization_id)
        if not allowed:
            logger.warning(
                "Unauthorized delete_channel by user %s for org %s",
                getattr(user, "id", None),
                organization_id,
            )
            return False
