...
async def test_global_admin_bypass():
    ...
    result = await auth.can(admin_user, "tournament:create", "tournament:*", organization_id=1)
...
async def test_organization_membership():
    ...
    result = await auth.can(user, "tournament:create", "tournament:*", organization_id=org.id)
...
async def test_builtin_roles():
    ...
    roles = await OrganizationRole.filter(organization=system_org, is_builtin=True).all()
...
async def test_authorization_service_api():
    ...
    # Test can() method
    can_create = await auth.can(admin_user, "tournament:create", "tournament:*", organization_id=1)
    logger.info("can() method returned: %s", can_create)
    
    # Test authorize() method (detailed result)
    result = await auth.authorize(admin_user, "tournament:create", "tournament:*", organization_id=1)
    logger.info("authorize() method returned: allowed=%s, reason=%s", result.allowed, result.reason)
...
async def test_caching():
    ...
    result1 = await auth.can(admin_user, "tournament:create", "tournament:*", organization_id=1)
    
    # Second call - should hit cache
    logger.info("Second authorization check (should use cache)...")
    result2 = await auth.can(admin_user, "tournament:create", "tournament:*", organization_id=1)
    ...
    result3 = await auth.can(admin_user, "tournament:create", "tournament:*", organization_id=1)
