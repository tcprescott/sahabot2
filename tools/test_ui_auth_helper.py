...
async def test_ui_auth_helper():
    ...
    try:
        ...
        if not org:
            print(
                "❌ No organizations found. Run tools/backfill_builtin_roles.py first."
            )
            return
        ...
        manager_user = await User.filter(
        ...
        if not all([admin_user, manager_user, member_user]):
            print(
                "❌ Test users not found. Run tools/test_tournament_service_auth.py first."
            )
            return
        ...
        print(
            f"   can_manage_async_tournaments: {admin_perms.can_manage_async_tournaments}"
        )
        ...
        print(
            f"   can_manage_scheduled_tasks: {admin_perms.can_manage_scheduled_tasks}"
        )
        print(
            f"   can_manage_race_room_profiles: {admin_perms.can_manage_race_room_profiles}"
        )
        ...
        print(
            f"   can_manage_async_tournaments: {manager_perms.can_manage_async_tournaments}"
        )
        ...
        assert (
            not manager_perms.is_organization_admin
        ), "Manager should NOT be org admin"
        ...
        assert (
            not manager_perms.can_manage_organization
        ), "Manager should NOT manage org"
        ...
        print(
            f"   can_view_organization_settings: {member_perms.can_view_organization_settings}"
        )
        ...
        assert (
            not member_perms.can_manage_tournaments
        ), "Member should NOT manage tournaments"
        ...
        assert (
            member_perms.can_view_organization_settings
        ), "Member should view org settings"
        ...
        if not org2:
            ...
            member2 = await OrganizationMember.create(
                organization_id=org2.id, user_id=admin_user.id
            )
            # Assign Organization Admin role
            admin_role = await OrganizationRole.filter(
                organization_id=org2.id, name="Organization Admin", is_builtin=True
            ).first()
            if admin_role:
                await OrganizationMemberRole.create(
                    member_id=member2.id, role_id=admin_role.id
                )

        batch_perms = await helper.get_multiple_organization_permissions(
            admin_user, [org.id, org2.id]
        )

        print(
            f"   Org {org.id} permissions: {batch_perms[org.id].is_organization_member}"
        )
        print(
            f"   Org {org2.id} permissions: {batch_perms[org2.id].is_organization_member}"
        )
        ...
        assert (
            not non_member_perms.is_organization_member
        ), "Non-member should NOT be org member"
        assert (
            not non_member_perms.can_manage_tournaments
        ), "Non-member should NOT manage tournaments"
        assert (
            not non_member_perms.can_view_tournaments
        ), "Non-member should NOT view tournaments"
