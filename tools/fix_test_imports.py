...
MOCK_PATH_MAPPINGS = {
    ...
    r"@patch\('application\.services\.task_scheduler_service\.":
        r"@patch('application.services.tasks.task_scheduler_service.",
...
def fix_test_file(file_path: Path) -> int:
    ...
    for old_pattern, new_pattern in MOCK_PATH_MAPPINGS.items():
        ...
        if count > 0:
            content = new_content
            replacements += count
            print(f"  {file_path.relative_to(file_path.parent.parent.parent)}: Replaced {count} occurrence(s) of '{old_pattern[:50]}...'")
...
def main():
    ...
    if files_updated > 0:
        ...
        print("3. Commit if successful: git add tests/ && git commit -m 'Fix test mock import paths'")
    else:
