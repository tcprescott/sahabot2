...
async def get_or_create_test_org():
    ...
    if not org:
        raise RuntimeError(
            "No organizations found in database - please create one first"
        )
...
async def create_test_users(org: Organization):
    ...
    if not admin_user:
        ...
        admin_role = await OrganizationRole.filter(
            organization=org, name="Admin", is_builtin=True
        ).first()
        if admin_role:
    ...
    if not manager_user:
        ...
        manager_role = await OrganizationRole.filter(
            organization=org, name="Tournament Manager", is_builtin=True
        ).first()
        if manager_role:
            await OrganizationMemberRole.create(
                member=await OrganizationMember.filter(
                    user=manager_user, organization=org
                ).first(),
                role=manager_role,
            )
...
async def test_create_tournament(
    service: TournamentService, org: Organization, users: dict
):
    ...
    if tournament:
        logger.info(
            "✓ Tournament Manager successfully created tournament (id=%s)",
            tournament.id,
        )
...
async def test_list_tournaments(
    ...
    if len(tournaments) == 0:
        ...
        logger.warning(
            "Regular member listed %s tournaments (might be expected based on policy)",
            len(tournaments),
        )
    ...
    if len(tournaments) == 0:
        ...
        logger.error(
            "✗ Non-member incorrectly allowed to list %s tournaments", len(tournaments)
        )
