...
async def create_system_organization():
    ...
    if existing:
        logger.info("System organization already exists (id=%s, name=%s)", existing.id, existing.name)
        return existing
    ...
    logger.info("System organization created: id=%s, name=%s", system_org.id, system_org.name)
    return system_org
...
async def create_builtin_roles(organization: Organization):
    ...
    logger.info(
        "Built-in roles creation complete: %s created",
        len(created_roles)
    )
async def verify_system_setup():
    ...
    logger.info("✓ System organization exists (id=%s, name=%s)", system_org.id, system_org.name)
    
    # Verify built-in roles exist
    roles = await OrganizationRole.filter(organization=system_org, is_builtin=True).all()
    role_names = [r.name for r in roles]
    
    expected_roles = {"User Manager", "Organization Manager", "Analytics Viewer", "Platform Moderator"}
...
async def main():
    ...
    try:
        ...
        logger.error("Error during System organization initialization: %s", str(e), exc_info=True)
        return 1
