...
async def update_builtin_role_actions():
    ...
    for org in organizations:
        ...
        for role_def in builtin_definitions:
            ...
            if not role:
                logger.warning("  - Role '%s' not found (should have been created by backfill)", role_def.name)
                continue
        ...
        logger.info("  Organization total: %s updated, %s skipped", org_updated, org_skipped)
        logger.info("")
